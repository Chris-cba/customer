DROP VIEW TRANSINFO.XODOT_IATN_V;

/* Formatted on 24/09/2013 15:32:02 (QP5 v5.215.12089.38647) */
CREATE OR REPLACE FORCE VIEW TRANSINFO.XODOT_IATN_V
(
   HWY,
   BEGIN_MP,
   HIGHWAY_NUMBER,
...
AS
   SELECT HWY,
     ...
     FROM (SELECT b.ne_unique HWY,
                  ...
                  DECODE (s.HIGHWAY_EA_NUMBER,
                          NULL, s2.HIGHWAY_EA_NUMBER,
                          s.HIGHWAY_EA_NUMBER)
                     SECTION_EA,
                  -- g.maint_section_crew_id SECTION_CREW,
                  -- g.maint_dist_id DISTRICT,
            ...
            WHERE     c.ne_id_of = b.nm_ne_id_of
                  ...
                  AND c.nm_begin_mp <= s2.nm_end_mp(+)) iatn,
          XODOT_EA_CW_DIST_REG_LOOKUP g
    WHERE section_EA = g.ea_number;


DROP PUBLIC SYNONYM XODOT_IATN_V;

CREATE OR REPLACE PUBLIC SYNONYM XODOT_IATN_V FOR TRANSINFO.XODOT_IATN_V;
