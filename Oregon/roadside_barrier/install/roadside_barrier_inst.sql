...
--   PVCS Identifiers :-
--
--       pvcsid                 : $Header:   //vm_latest/archives/customer/Oregon/roadside_barrier/install/roadside_barrier_inst.sql-arc   1.0   Oct 22 2010 10:54:24   Ian.Turnbull  $
--       Module Name      : $Workfile:   roadside_barrier_inst.sql  $
--       Date into PVCS   : $Date:   Oct 22 2010 10:54:24  $
...
BEGIN
   --
      IF USER IN ('SYS','SYSTEM')
       THEN
         RAISE_APPLICATION_ERROR(-20000,'You cannot install Roadside barrier views by exor as ' || USER);
      END IF;
END;      
/
...
select decode(upper('&ok_res'),'Y','&exor_base'||'roadside_barrier'||'&terminator'||
        'install'||'&terminator'||'roadside_barrier_install','exit') run_file
from dual
/
