...
--   PVCS Identifiers :-
--
--       pvcsid                 : $Header:   //vm_latest/archives/customer/Oregon/gasb34/install/gasb34_inst.sql-arc   3.0   Sep 15 2010 09:57:58   ian.turnbull  $
--       Module Name      : $Workfile:   gasb34_inst.sql  $
...
prompt
prompt About to install gasb34 extract using exor base : &exor_base
prompt
accept ok_res prompt "OK to Continue with this setting ? (Y/N) "

select decode(upper('&ok_res'),'Y','&exor_base'||'gasb34'||'&terminator'||
        'install'||'&terminator'||'gasb34_install','exit') run_file
from dual
/
