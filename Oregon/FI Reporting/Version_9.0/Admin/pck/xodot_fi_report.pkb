...
--   PVCS Identifiers :-
--
--       pvcsid                 : $Header:   //new_vm_latest/archives/customer/Oregon/FI Reporting/Version_9.0/Admin/pck/xodot_fi_report.pkb-arc   1.0   Jan 15 2016 17:55:44   Sarah.Williams  $
...
BEGIN
   ...
   FOR i IN get_crew_types LOOP   ---- Get he distinct crew types to be processed
      ...
      FOR i2 IN get_activity_data(i.crew_type) LOOP
		If (i2.asset_type = 'ELCS' and i2.Calculation_type = 'SUMMARY' and SUBSTR(i2.HWY,7,1) = 'Z') then --Removing Extra Sumarrized Z-Mileage Items, as an extra if isn't needed Above, Re: PPT#283*****
			null;
		else
