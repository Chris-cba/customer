...
--   PVCS Identifiers :-
--
--       pvcsid                 : $Header:   //new_vm_latest/archives/customer/Oregon/FI Reporting/Version_X.0/Admin/pck/xodot_fi_report.pkb-arc   1.0   Jan 15 2016 18:08:54   Sarah.Williams  $
    ...
    (
       ...
       -- turnlanes end
        ...
        where
           ...
           --and R.MILEAGE_TYPE <> 'P'
           and   R.SCNS_HWY_EA_NO in
                (
                      select HIGHWAY_EA_NUMBER from v_nm_seea_nt
                      where ne_unique not like '%TEMP%'
...
--
 ...
 SELECT  lookup_crew crew_unique
   ...
   select p_crew_type lookup_crew_type
   ...
   and hact.crew_type =--- 'SECW'
                         p_crew_type
                                                  --*****************************************
                                                  -- debug
                                     --               and hact.calculation_type =  'CALCULATED'
                                                -- end debug
                                                --*********************************
  -- and hact.asset_type = count_sections.iit_inv_type
   and count_sections.iit_inv_type   =  --'SCNS'
    ...
    -- and crew.lookup_crew = 4208 -- debug
   -- and hact.feature_type = 'ROADWAY MILES' --debug
   order by iit_ne_id;
...
--
  ...
  --and a.feature_type = 'ROADWAY MILES' -- debug
   --and a.highway like '00100I%'  --Debug
  --
 -- and A.CREW = 4208 -- debug
   ORDER BY crew;
...
BEGIN
		...
		--Z mileage should only have a value other than 0 if the calculation type is length or calculation, PP#283
   ...
   FOR i IN get_data LOOP
	  ...
	  IF i.calculation_type = 'SUMMARY' THEN
		 ...
		 if SUBSTR(i.Highway,7,1) = 'Z' then l_feature_count := 0; End if;  --PP#283
		 ...
		 and highway = i.Highway  -- added for --PP#283
		 ;
...
FUNCTION length_process (p_asset_type nm_inv_items.iit_inv_type%TYPE
						...
						,p_attribute_values V_NM_HACT.attribute_values%TYPE
                        ,p_hwy nm_elements.ne_unique%type
                        ,p_xsp_values VARCHAR2 ) return NUMBER IS
...
BEGIN
   ...
   if l_xsp_values is not null then
	...
	IF p_attribute_name IS NOT NULL AND p_attribute_values is not null THEN
          ...
          l_attrib := ' and ' || p_attribute_name || ' in ( '  || l_attribute_values  || ') ';

    end if;

    v_query := '

         select sum (least(b.nm_end_mp, cs.nm_end_mp) - greatest(b.nm_begin_mp, cs.nm_begin_mp))
   from (
        ...
        ,(
                    select nm_ne_id_of hwy_ne_id_of from nm_members
                    where nm_ne_id_in = nm3net.get_ne_id(''' || p_hwy || ''',''HWY'')
        ) hwy
        ,(
...
nm_debug.debug_on;
 ...
 GET_CREW_ACT;
  ...
  create_fi_views;  -- Comminted for debug
