    ...
    FROM 
        (SELECT route_id 
            ...
            ELSE 
                1 
            END break_point 
        FROM V_HPMS_7_581_HPMS_Sample_ID) 
) 
 GROUP BY route_id, group_by_fld, value_text 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE VIEW "TRANSINFO"."V_OHMS_FINAL_7_610_PEAK_LANES" ("ROUTE_ID", "BEGIN_POINT", "END_POINT", "SECTION_LENGTH") AS 
  SELECT route_id 
    , min(begin_point) begin_point 
    , max(end_point) end_point 
    , max(end_point) - min(begin_point) section_length  
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FORCE VIEW "TRANSINFO"."V_OHMS_FINAL_7_611_COUNTER_PEA" ("ROUTE_ID", "BEGIN_POINT", "END_POINT", "SECTION_LENGTH") AS 
  SELECT route_id 
    , min(begin_point) begin_point 
    , max(end_point) end_point 
    , max(end_point) - min(begin_point) section_length  
