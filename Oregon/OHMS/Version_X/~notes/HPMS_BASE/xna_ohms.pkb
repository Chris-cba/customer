...
/*
	...
	data stored in the HPMS_ tables.
...
-- PVCS Identifiers :-
--
-- pvcsid : $Header:   //new_vm_latest/archives/customer/Oregon/OHMS/Version_X/~notes/HPMS_BASE/xna_ohms.pkb-arc   1.0   Jan 15 2016 20:52:04   Sarah.Williams  $
-- Module Name : $Workfile:   xna_ohms.pkb  $
-- Date into PVCS : $Date:   Jan 15 2016 20:52:04  $
-- Date fetched Out : $Modtime:   Aug 19 2011 21:46:06  $
...
-- Write to the activity log
							...
							t_variables_line		:= t_newline || t_tab ||  t_tab || 'SELECT hpms_hal_id.nextval INTO t_hal_id FROM DUAL;' || t_newline;
							...
							t_variables_line		:= NULL;
							t_variables_line		:= t_newline || t_tab ||  t_tab || 'INSERT INTO HPMS_ACTIVITY_LOG(HAL_ID, HAL_DATE, HAL_USER, HAL_ITEM, HAL_TABLE_ID, HAL_STATUS)';
...
BEGIN
	...
	t_variables_line		:= t_newline || t_tab ||  t_tab || 'INSERT INTO HPMS_ACTIVITY_LOG(HAL_ID, HAL_DATE, HAL_USER, HAL_ITEM, HAL_TABLE_ID, HAL_STATUS, HAL_MESSAGE)';
	t_procedure_line_length		:= length(t_variables_line);
	t_procedure_line		:= t_variables_line;
	t_procedure_line_length		:= length(t_procedure_line);
	dbms_lob.writeappend(t_procedure, t_procedure_line_length, t_procedure_line);
			...
			IF nvl(t_template_return_pos,-5) NOT IN (-5,0) THEN
				...
				ELSIF instr(t_template_line,'<<sub_view_points>>') > 0 THEN
					FOR t_loop IN 1..tab_item.count LOOP
						...
						t_viewname	:= substr('V_HPMS_FINAL_' || t_ht_id || '_' || tab_item(t_loop) || '_' || replace(t_item_desc,' ','_'),1,30);
				...
				IF nvl(t_output,'-x') = '-x' THEN
					...
					FOR c_map IN cur_map_all(t_table_id, t_data_item) LOOP
						IF c_map.hcl_name = 'ROUTE_ID' THEN
							...
							t_route_table			:= 'HPMS_TMP_RTE_ITEM_' || c_map.hm_item;
						END IF;
					END LOOP;
					...
					tab_table(1)				:= 'HPMS_TMP_' || t_table_id || 
											'_' || t_data_item || '_' || t_primary_item;
					tab_rte_table(1)			:= 'HPMS_TMP_RTE_' || t_table_id || 
											'_' || t_data_item || '_' || t_primary_item;
					t_aggregate_seg_table			:= 'HPMS_TMP_SEG_' || t_table_id || '_' || t_data_item || '_' || t_primary_item;
					t_aggregate_detail_table		:= 'HPMS_TMP_DETAIL_' || t_table_id || '_' || t_data_item || '_' || t_primary_item;
					...
					FOR c_adddl_items IN cur_aggregate_item_list (t_table_id, t_data_item) LOOP
						...
						IF t_found = 0 THEN
							...
							tab_table(t_counter)		:= 'HPMS_TMP_' || t_table_id || 
											'_' || t_data_item || '_' ||  c_adddl_items.hm_item;
							tab_rte_table(t_counter)	:= 'HPMS_TMP_RTE_' || t_table_id || 
											'_' || t_data_item || '_' ||  c_adddl_items.hm_item;
						END IF;
					END LOOP;
		...
		BEGIN
			...
			IF nvl(t_template_return_pos,-5) NOT IN (-5,0) THEN
				...
				ELSIF instr(t_template_line,'<<route_id_options>>') > 0 THEN
					...
					SELECT DISTINCT qpr_order BULK COLLECT INTO tab_item FROM hpms_primary_route ORDER BY qpr_order;
					...
					FOR i IN 1..tab_item.count LOOP
						...
						FROM DUAL;
  						...
  						SELECT max(qpr_route_identifier) INTO t_primary_item FROM hpms_primary_route WHERE qpr_order = tab_item(i);
				...
				ELSIF instr(t_template_line,'<<sub_view_points>>') > 0 THEN
					...
					SELECT DISTINCT qpr_linear_group BULK COLLECT INTO tab_item FROM hpms_primary_route WHERE qpr_linear_group IS NOT NULL;
...
--						END IF;
				...
				ELSIF instr(t_template_line,'<<inner from network restriction>>') > 0 THEN
					IF t_network_flag > 0 THEN
						t_output	:= t_output || t_whitespace || ', hpms_' || t_table_id || '_network_mv b' || t_newline;
					END IF;
...
-- Create a TABLE that holds the datum/offsets that the results will be restricted to
	...
	t_network_mv_name	:= 'HPMS_' || trim(t_ht_id) || '_NETWORK_MV';
...
BEGIN
	...
	BEGIN
		...
		t_view			:= substr('V_HPMS_' || t_table_id || '_' || t_data_item || '_' || replace(t_item_desc,' ','_'),1,30);
		create_hpms_view(t_item_sql, t_item_desc, t_table_id, t_data_item, t_view,'TEMP');
		...
		t_item_sql			:= replace(t_route_sql,'CURSOR CUR_' || t_data_item || ' IS',' ');
		t_view				:= substr('V_HPMS_FINAL_' || t_table_id || '_' || t_data_item || '_' || replace(t_item_desc,' ','_'),1,30);
		create_hpms_view(t_item_sql, t_item_desc, t_table_id, t_data_item, t_view, 'FINAL');
...
t_hal_id		HPMS_ACTIVITY_LOG.HAL_ID%TYPE;
t_hal_date		HPMS_ACTIVITY_LOG.HAL_DATE%TYPE;
t_hal_user		HPMS_ACTIVITY_LOG.HAL_USER%TYPE;
t_hal_item		HPMS_ACTIVITY_LOG.HAL_ITEM%TYPE;
t_hal_table_id		HPMS_ACTIVITY_LOG.HAL_TABLE_ID%TYPE;
t_hal_error		HPMS_ACTIVITY_LOG.HAL_ERROR%TYPE;
t_hal_item		HPMS_ACTIVITY_LOG.HAL_MESSAGE%TYPE;
...
BEGIN
	...
	IF t_clob_length > 100 THEN
		...
		t_procedure_name		:= 'XNA_HPMS_POPULATE_'|| t_table_id;
		t_error_count			:= 0;
...
PROCEDURE hpms_params IS
   c_this_module  CONSTANT hig_modules.hmo_module%TYPE := 'HPMS_EX';
...
BEGIN
  ...
  htp.formopen(curl => g_package_name || '.hpms_report');
...
PROCEDURE hpms_report(pi_report_type varchar2) IS
...
BEGIN
	...
	nm3web.module_startup(pi_module => c_this_module);
  	...
  	if pi_report_type='HPMS1' then
    		l_rec_nuf.name     := 'HPMS_SECTIONS_'||to_char(sysdate,'DD-MON-YYYY_HH24_MI_SS')||'.csv';
    		...
    		for c1rec in (SELECT
					...
					|| '|' || VALUE_DATE
					|| '|' || COMMENTS v_row
				FROM HPMS_SUBMIT_SECTIONS) LOOP
       		l_tab(l_tab.count+1)	:= c1rec.v_row ||chr(10);
       		l_rec_nuf.doc_size  	:= l_rec_nuf.doc_size+length(l_tab(l_tab.count));
    		end loop;
  	elsif pi_report_type='HPMS2' then
    		l_rec_nuf.name     := 'HPMS_SAMPLES_'||to_char(sysdate,'DD-MON-YYYY_HH24_MI_SS')||'.csv';
		...
		NULL;
/*
    		l_rec_nuf.name     := 'HPMS_'||to_char(sysdate,'DD-MON-YYYY_HH24_MI_SS')||'.csv';
    		...
    		for c1rec in (SELECT
					...
					|| '|' || ROUTE_ID
					|| '|' || SHAPE v_row
				FROM HPMS_ROUTE_SHAPES) LOOP
       		l_tab(l_tab.count+1)	:= c1rec.v_row ||chr(10);
       		l_rec_nuf.doc_size  	:= l_rec_nuf.doc_size+length(l_tab(l_tab.count));
    		end loop;
*/
	elsif  pi_report_type='ACTIVITY_LOG' then
     		l_rec_nuf.name    := 'HPMS_ACTLOG_'||to_char(sysdate,'DD-MON-YYYY_HH24_MI_SS')||'.csv';
    		for c1rec in (SELECT
					HAL_ID
					|| '|' || HAL_DATE
					|| '|' || HAL_USER
					...
					|| '|' || HAL_MESSAGE
					|| '|' || HAL_ERROR v_row
				FROM HPMS_ACTIVITY_LOG ORDER BY HAL_ID DESC) LOOP
      		l_tab(l_tab.count+1):=c1rec.v_row ||chr(10);
      		l_rec_nuf.doc_size  := l_rec_nuf.doc_size+length(l_tab(l_tab.count));
     		end loop;
  	end if;
...
BEGIN
	...
	IF t_global_log = 'YES' THEN
		...
		(
			...
			, t_table_id
			, t_status
			, 'Procedure XNA_HPMS_POPULATE_' || t_table_id || ' Generated'
		);
		COMMIT;
	END IF;
	...
	IF t_global_log = 'YES' THEN
		...
		(
			...
			, t_table_id
			, t_status
			, 'Compilation of XNA_HPMS_POPULATE_' || t_table_id || ' Completed'
		);
		COMMIT;
	END IF;
...
WHEN is_complete THEN
    RAISE_APPLICATION_ERROR( -20000,'Compilation of XNA_HPMS_POPULATE_' || t_table_id || ' Completed' );
...
WHEN OTHERS THEN
    ...
    raise_application_error( -20000,'Compilation attempt Failed,  Error is '
                            || t_error_text
                            || '. Please review the HPMS_ACTIVITY_LOG table.');
...
BEGIN
	...
	t_query	:= replace(t_query,'hpms_xxx',p_view_name);
