DROP MATERIALIZED VIEW TRANSINFO.V_NM_SHLD137_OUTER_MV_NW;
CREATE MATERIALIZED VIEW TRANSINFO.V_NM_SHLD137_OUTER_MV_NW 
TABLESPACE HIGHWAYS
PCTUSED    0
PCTFREE    10
INITRANS   2
...
WITH PRIMARY KEY
AS 
/* Formatted on 12/9/2010 4:43:46 PM (QP5 v5.115.810.9015) */
SELECT   NM_ADMIN_UNIT,
  ...
  FROM   (SELECT   ne_id_of, nm_begin_mp, nm_end_mp
         ...
         (SELECT   NM_ADMIN_UNIT,
                   ...
                   nm_end_mp
            FROM   v_nm_shld_nw
           WHERE   layer = 1 AND iit_x_sect = 'OS2I') a
 WHERE       r.ne_id_of = a.ne_id_of(+)
         AND r.nm_begin_mp < a.nm_end_mp(+)
         AND r.nm_end_mp > a.nm_begin_mp(+);

COMMENT ON MATERIALIZED VIEW TRANSINFO.V_NM_SHLD137_OUTER_MV_NW IS 'snapshot table for snapshot TRANSINFO.V_NM_SHLD137_OUTER_MV_NW';
