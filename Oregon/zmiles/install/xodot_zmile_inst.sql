...
prompt
prompt About to install Z Mileage using exor base : &exor_base
prompt
accept ok_res prompt "OK to Continue with this setting ? (Y/N) "

select decode(upper('&ok_res'),'Y','&exor_base'||'zmile'||'&terminator'||
        'install'||'&terminator'||'xodot_zmile_install','exit') run_file
from dual
/
