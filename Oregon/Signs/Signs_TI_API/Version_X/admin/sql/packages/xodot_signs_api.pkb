...
/*
	...
	UPDATE02:	Changed Error Logging to use the bigger truncation size , 2015.05.06, JMM
	UPDATE03:	Added Get Version Function and changed the name of change_att_val, 2015.05.12, JMM
	UPDATE04:	Quick incomplete fix, to get testing progressing again after metamodel change, 2015.05.12, JMM
	UPDATE05:	Implemented Fix after metamodel change,removed filters from some of the reports, 2015.05.14, JMM
	UPDATE05:	Implemented Fix after metamodel change, 2015.05.26, JMM
	UPDATE06:	Changed a few queries to help whit a speed issue and a EA Location issue., 2015.06.02, JMM
	UPDATE07:	Additional Comments have been added., 2015.06.08, JMM
	UPDATE08:	The Child get tasks cursor has changed, it was hanging with a large amount of records, 2015.06.08, JMM
	UPDATE09:	Change HWYNO in the hundredths report to include route suffix 5 digit if not 00 , 2015.06.25, JMM
	UPDATE10:	Change v_nm_SUPP When getting support information form TI -> SFA , 2015.07.14, JMM
*/
	g_version varchar(12)				:= '01_17';

	
/* ************************************************************
	Object:		xodot_signs_api
	Purpose:	The functions and procedures found in this package is intended to be used from the Sign_Broker. 
				The package contains an interface to TI that the broker can access, many calls are a front end for existing TI Api calls.  
				The package is not intended to be used directly in TI.  
	Notes:		
...
function find_ea_from_crew_number(s_crew_number in varchar2) return varchar2 is	---------------------------------------------------------------------------------------
	...
	/* ************************************************************
		Object:		Public function find_ea_from_crew_number
		Purpose:	This Function is used to find the EA number of the crew.  It returns a comma separated list of EA numbers for the imputed crew number
		Notes:		
		Created:	2015.06.05	J.Mendoza
	************************************************************* */
...
/* ************************************************************
	Object:		public function delete_child_asset
	Purpose:	This function deletes asset children without end dating them. 
	Notes:		This is Designed to be used for Sign assets only. 
	Created:	2015.06.05	J.Mendoza
************************************************************* */

function delete_child_asset(n_parent_ne_id number, s_asset_type varchar2, s_sign_type varchar2 default null) return varchar2 is	---------------------------------------------------------------------------------------
	s_return varchar2(50) := null;
	begin
		
		if s_asset_type <> 'SNSN' then 
	
			delete nm_members_all 
				where nm_ne_id_in in (select iit_ne_id from nm_inv_items_all where iit_foreign_key = n_parent_ne_id and iit_inv_type = s_asset_type);
		...
		else -- we need to be care with sign assets so that only the type we want is removed, custom/standard
			delete nm_members_all 
				where nm_ne_id_in in (select iit_ne_id from nm_inv_items_all where iit_foreign_key = n_parent_ne_id and iit_inv_type = s_asset_type and IIT_CHR_ATTRIB26 = s_sign_type);
			delete nm_inv_item_groupings_all 
				where iig_item_id in (select iit_ne_id from nm_inv_items_all where iit_foreign_key = n_parent_ne_id and iit_inv_type = s_asset_type and IIT_CHR_ATTRIB26 = s_sign_type);
			delete nm_inv_items_all
				where iit_foreign_key = n_parent_ne_id and iit_inv_type = s_asset_type and IIT_CHR_ATTRIB26 = s_sign_type;
		end if;
		return s_return;
...
/* ************************************************************
	Object:		public function get_version 
	Purpose:	returns the global version  variable
	Notes:		The broker uses it  to see if there is an error with this package, future versions may use the version number.
	Created:	2015.06.05	J.Mendoza
************************************************************* */
...
/* ************************************************************
	Object:		get_task_list_sign_assets
	Purpose:	This procedure retrieves a list and count of Sign asset objects that need updating, both parent and children for a specific sign crew.
	Notes:		
	Created:	2015.06.05	J.Mendoza
************************************************************* */
	...
	cursor c_asset_list(s_asset_list varchar2, s_ea_list varchar2, d_sync_date date) is
		with mem_inv_ea as
		( 
			select  /*+ materialize */ nm_ne_id_of from
				nm_members mem_inv_ea
				, ( 
					select * from nm_inv_items where iit_inv_type = 'SGNS'
					and iit_itemcode in  
				...
				where 	b.iit_ne_id = mem_inv_ea.nm_ne_id_in 
		)
		select mem_inv_on_datum.nm_obj_type asset, count(*) cnt from 
				...
				where 1=1
				 --and   mem_inv_ea.nm_obj_type = 'SGNS'				
                ...
                and   	mem_inv_on_datum.nm_obj_type in  (    --THis XML sting converts a comma separated list to a table                
                            	SELECT to_char(trim(EXTRACT(column_value,'/e/text()'))) myrow from (select  'SNIN' col1 from dual) x,
                            	TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<ROW><e>'||REPLACE(col1,',','</e><e>')||'</e></ROW>'),'//e')))
                        				)                 
				...
				and decode(trunc(mem_inv_on_datum.nm_date_modified),trunc(d_sync_date),decode(mem_inv_on_datum.nm_modified_by,user,0,1),1) =1  -- remove the items by this user on the last sync date
				and decode(trunc(inv_on_datum.iit_date_modified),trunc(d_sync_date),decode(inv_on_datum.iit_modified_by,user,0,1),1) =1  -- remove the items by this user on the last sync date
                group by mem_inv_on_datum.nm_obj_type;
				
				
				--Cursor changed due to long run time.  A date index was also added
	...
	cursor c_ch_asset_list(s_asset_list varchar2, s_ea_list varchar2, d_sync_date date) is
		select ch.iit_inv_type asset, count(*) cnt from 
			(select inv_on_datum.iit_ne_id from 
				...
				and decode(trunc(ch.iit_date_modified),trunc(d_sync_date),decode(ch.iit_modified_by,user,0,1),1) =1  -- remove the items by this user on the last sync date
	...
	cursor c_ch_asset_list(s_asset_list varchar2, s_ea_list varchar2, d_sync_date date) is
		with mem_inv_ea as
		(
			select  /*+ materialize */  m.nm_ne_id_of from nm_members m
				, (
					select * from nm_inv_items 
						where iit_inv_type = 'SGNS'
						and iit_itemcode in  
						(    --THis XML sting converts a comma separated list to a table                
							SELECT to_char(trim(EXTRACT(column_value,'/e/text()'))) myrow from (select  s_ea_list col1 from dual) x,
							TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<ROW><e>'||REPLACE(col1,',','</e><e>')||'</e></ROW>'),'//e')))
				...
				where  	b.iit_ne_id = m.nm_ne_id_in 
		)
		, t_asset_list as 
		(    --THis XML sting converts a comma separated list to a table                
			SELECT to_char(trim(EXTRACT(column_value,'/e/text()'))) myrow from (select  s_asset_list col1 from dual) x,
		...
		--
		--
		select iit_inv_type asset, count(*) cnt from		
			(select iit_inv_type,iit_modified_by ,inv_on_datum.iit_ne_id,IIT_date_modified from             		
					 nm_members_all mem_inv_on_datum
					, nm_inv_items_all inv_on_datum
					,mem_inv_ea
				where 1=1				
				and     mem_inv_ea.nm_ne_id_of =  mem_inv_on_datum.nm_ne_id_of 				      
				and   	mem_inv_on_datum.nm_obj_type in (select * from t_asset_list)                 
				and 	inv_on_datum.iit_ne_id = mem_inv_on_datum.nm_ne_id_in
				and inv_on_datum.IIT_date_modified >=d_sync_date				
				and decode(trunc(inv_on_datum.iit_date_modified),trunc(d_sync_date),decode(inv_on_datum.iit_modified_by,user,0,1),1) =1  -- remove the items by this user on the last sync date
			)
			group by iit_inv_type;
...
/* ************************************************************
	Object:		procedure snin_opps
	Purpose:	Used for: operations to the asset type SNIN, Sign Installations.  This is need so that special error logging can occur.
	Notes:		This is just an extension of a TransInfo generated Api used for Sign Broker error logging
	Created:	2015.06.05	J.Mendoza
************************************************************* */
...
procedure snin_opps				( t_result 				out		xodot_signs_asset_op
	...
	type l_loc_info is record(ne_id_of number(10), slk number(10,3));
	
	r_placement_info l_loc_info;
	
	function get_loc_datum(s_lrm varchar2, n_mp number) return l_loc_info is
		/*
			Used for: Working around a location issue when the sign is being located on the end of a datum with a distance break
		*/
		/* ************************************************************
			Object:		private function get_loc_datum
			Purpose:	Working around a location issue with the MP placement in the generated API when the sign is being located on the end of a datum with a distance break
			Notes:		
			Created:	2015.06.05	J.Mendoza
		************************************************************* */
		...
		n_count number;
		--ex_loc	Exception;
		--ex_text varchar2(500);
		r_loc l_loc_info;
		begin
			select count(*) into n_count from  nm_members a, nm_elements b where nm_ne_id_in = nm3net.get_ne_id(s_lrm,'HWY')
					and  n_mp between nm_slk and nm_end_slk
					and b.ne_id = a.nm_ne_id_of
					and ne_type <> 'D'
					and rownum=1
					order by a.nm_slk desc;
		if n_count = 1 then		
			select nm_ne_id_of, (n_mp - nm_slk) mp   into r_loc.ne_id_of, r_loc.slk from  nm_members a, nm_elements b where nm_ne_id_in = nm3net.get_ne_id(s_lrm,'HWY')
					and  n_mp between nm_slk and nm_end_slk
					and b.ne_id = a.nm_ne_id_of
					and ne_type <> 'D'
					and rownum=1
			...
			raise_application_error(-20001, 'Error when trying to locate this asset, please confirm the location exists.');
		end if;
		return r_loc;
	...
	/* ************************************************************
	Object:		private function changed_att_val
	Purpose:	To get the original value of asset before updating it.  If the input from the broker is null
	Notes:		
	Created:	2015.06.05	J.Mendoza
	************************************************************* */
	function changed_att_val(n_ne_id number, col varchar2, inputval varchar2) return varchar2 is
	...
	begin
		...
		case upper(s_operation)
			...
			--Changed location code b/c of ambiguous mp error, put it into a function in case it needs to be expanded on to keep this bit of code clean.
			r_placement_info := get_loc_datum(s_lrm,n_MP);
			
			--xodot_signs_logging.log_exception(p_iit_ne_id, s_sfa_id ,s_LRM,n_MP,000000,'DEBUG: get_loc_datum(s_lrm,n_MP)' ||r_placement_info.ne_id_of || ', ' || r_placement_info.slk); --for debugging
				 ...
				 n_ne_id:= nm3api_inv_snin.ins (
							...
							,pf_sign_inv_side_of_rd			=> pf_sign_inv_side_of_rd
							,pf_sign_inv_trvl_dir  			=> pf_sign_inv_trvl_dir  
							...
							--,p_element_mp                   =>     n_MP
							,p_element_ne_id              => r_placement_info.ne_id_of
              				,p_element_mp                 => r_placement_info.slk
							-- </LOCATION DETAILS>
							);
...
*/
			...
			when 'U'  then
				...
				nm3api_inv_snin.date_track_upd_attr (
							...
							,p_effective_date   			 	=> nm3user.get_effective_date
							-- <FLEXIBLE ATTRIBUTES>
							,pf_loc_note                    	=> changed_att_val(p_iit_ne_id, 'iit_chr_attrib27', pf_loc_note)    
							,pf_city_rd_flg                 	=> changed_att_val(p_iit_ne_id, 'iit_chr_attrib28', pf_city_rd_flg)    
							,pf_cnty_rd_flg                 	=> changed_att_val(p_iit_ne_id, 'iit_chr_attrib29', pf_cnty_rd_flg)
							,pf_sign_inv_side_of_rd				=> pf_sign_inv_side_of_rd
							,pf_sign_inv_trvl_dir  				=> pf_sign_inv_trvl_dir  
							,pf_off_netwrk_note             	=> changed_att_val(p_iit_ne_id, 'iit_chr_attrib66', pf_off_netwrk_note)    
							,pf_dstnc_from_pvmt             	=> changed_att_val(p_iit_ne_id, 'iit_distance'	 , pf_dstnc_from_pvmt)     	
							,pf_lat                         	=> changed_att_val(p_iit_ne_id, 'iit_num_attrib100',pf_lat)   
							,pf_longtd                      	=> changed_att_val(p_iit_ne_id, 'iit_num_attrib101',pf_longtd)   
                              -- </FLEXIBLE ATTRIBUTES>
                              );
			when 'E'  then
	...
	exception when others then
		...
		xodot_signs_logging.log_exception(p_iit_ne_id, s_sfa_id ,s_LRM,n_MP,SQLCODE,'SNIN: ' || substr(SQLERRM,1,2000));
...
/* ************************************************************
	Object:		procedure snsn_opps
	Purpose:	Used for: operations to the asset type SNSN, Sign "Signs" (Panels).  This is need so that special error logging can occur
	Notes:		This is just an extension of a TransInfo generated Api used for Sign Broker error logging, 
				the Delete Feature removes it form the tables it does not endd ate the item
	Created:	2015.06.05	J.Mendoza
************************************************************* */
...
PROCEDURE snsn_opps 			(t_result 						out		xodot_signs_asset_op
	...
	Begin
		...
		case upper(s_operation)
			...
			when 'D' then                                               
				s_temp := delete_child_asset(p_iit_ne_id, 'SNSN', pf_sign_typ);
		end case;
	...
	exception when others then
		...
		xodot_signs_logging.log_exception(p_iit_ne_id, s_sfa_id ,null,null,SQLCODE,substr(SQLERRM,1,2000));
		nm3user.set_effective_date(sysdate);
		t_result := s_return_val;
end snsn_opps;
...
/* ************************************************************
	Object:		procedure snsu_opps
	Purpose:	Used for: operations to the asset type SNSU, Sign supports.  This is need so that special error logging can occur
	Notes:		This is just an extension of a TransInfo generated Api used for Sign Broker error logging, 
				the Delete Feature removes it form the tables it does not endd ate the item
	Created:	2015.06.05	J.Mendoza
************************************************************* */
...
/* ************************************************************
	Object:		procedure snml_opps
	Purpose:	Used for: operations to the asset type SNML, Sign Maintenance Log.  This is need so that special error logging can occur
	Notes:		This is just an extension of a TransInfo generated Api used for Sign Broker error logging, 
				the Delete Feature removes it form the tables it does not end date the item
	Created:	2015.06.05	J.Mendoza
************************************************************* */
		...
		case upper(s_operation)
			when 'I' then
				n_ne_id := nm3api_inv_snml.ins (
					...
					,pf_sign_facing_note      		=>		pf_sign_facing   
...
/* ************************************************************
	Object:		get_sign_assets
	Purpose:	To fetch a list of sign assets that have changed since a certain date based on crew number and type.
	Notes:		It outputs a sys_refcursor
	Created:	2015.06.05	J.Mendoza
************************************************************* */
	...
	/*	.
		open ref_cur for
			...
			select inv_on_datum.* from 
				...
				and decode(trunc(mem_inv_on_datum.nm_date_modified),trunc(d_date_last_synced),decode(mem_inv_on_datum.nm_modified_by,user,0,1),1) =1  -- remove the items by this user on the last sync date
			union all
			select inv_on_datum.* from 
            		mem_inv_ea
                    , nm_members_all mem_inv_on_datum
				...
				and decode(trunc(inv_on_datum.iit_date_modified),trunc(d_date_last_synced),decode(inv_on_datum.iit_modified_by,user,0,1),1) =1  -- remove the items by this user on the last sync date
				)
                ;
...
/* ************************************************************
	Object:		get_asset_lov
	Purpose:	Used for:  Obtaining List of values for the Broker that are stored as assets in TI.  This reads directly from the inventory Item table
	Notes:		
...
*/
		...
		if n_count >0 then
			open ref_cur for
				select iit_inv_type, iit_end_date
					...
					,IIT_NUM_ATTRIB102
					,IIT_NUM_ATTRIB103
					, case iit_inv_type when 'SIGN' then nvl((select graph_id from v_nm_sngr where graph_nm = IIT_CHR_ATTRIB31),1) else null end graph_id
					, case iit_inv_type when 'SNSU' then nvl((select SUPP_DESC from v_nm_supp where supp_typ = IIT_CHR_ATTRIB26),1) else null end supp_desc
	...
	exception when others then
		...
		xodot_signs_logging.log_exception(null, null ,null,null,SQLCODE,substr(SQLERRM,1,2000));
...
/* ************************************************************
	Object:		get_domain_lov
	Purpose:		Used for:  Obtaining List of values for the Broker that are stored as Domains in TI, this only reads Inv Item Domains.
	Notes:		
	Created:	2015.06.08	J.Mendoza
************************************************************* */
...
/* ************************************************************
	Object:		get_mem_location_from_ne_id(
	Purpose:	Obtaining the location of an installation 
	Notes:		Takes an NEID of an SNIN asset and gets the HWY and MilePoint of where it is located
	Created:	2015.06.08	J.Mendoza
************************************************************* */
procedure get_mem_location_from_ne_id(n_ne_id in number, ref_cur out sys_refcursor) is
/*
	Used for:  Obtaining the location of an installation  
*/
	begin
		open ref_cur for
			select ne_loc.ne_unique, ne_owner hwy_num, ne_name_1 MT , ne_name_2 overlap_code,ne_prefix roadway_ID ,ne_number gen_direct ,ne_sub_type link_id , nm_loc.nm_slk+nm_in.nm_begin_mp mp,  nm_loc.* 
	...
	/* ************************************************************
	Object:		get_child_from_parent_ne_id
	Purpose:	Used for:  Obtaining the children assets of an SNIN installation
	Notes:		Given an SNIN NEID returns the child of an imputed type for that parent.
	Created:	2015.06.08	J.Mendoza
************************************************************* */
...
*/
	begin
		open ref_cur for
		select a.*
		, case a.iit_inv_type when 'SNSU' then nvl((select SUPP_DESC from v_nm_supp where supp_typ = IIT_CHR_ATTRIB26 and rownum = 1),1) else null end supp_desc
		from nm_inv_items a, NM_INV_ITEM_GROUPINGS 
...
/* ************************************************************
	Object:		get_simple_report_lookup
	Purpose:	Used for:  Obtaining List of values for the Broker that are stored in the reports schema for TI, 
				May also be used for more complicated reporting in the TI schema.
	Notes:		
	Created:	2015.06.08	J.Mendoza
************************************************************* */
	...
	begin
		case upper(s_report)
			when 'TBLHIGHWAYEA' then
				s_sql := 'select * from reports.TIODS_MAINT_HIER a where crew_typ = '''|| s_crew_type || '''';
				--' and (#CREW#)';
				/*  Filtering Removed for this report.
				if upper(b_crew_only) = 'TRUE' then
					s_where_replace := 'crew = ''' ||s_crew || '''';  
					
					
				else
					s_where_replace := 'lpad(regexp_replace( dist, ''[^[:digit:]]'', null ),2,0) = ''' || s_district || '''' || 'or crew = ''' ||s_crew || ''''; --'
				end if;
			...
			when 'TBLHIGHWAYS' then
				s_sql := q'{select  distinct regexp_replace( substr(lrm_key,1,5), '00$', null ) hwy, hwy_nm from  reports.TIODS_RTE_HWY_LOC where trunc(extrct_prc_dt) >=  '}' || trunc(d_last_sync_date) ||''' order by 1';
			...
			when 'TBLHWYGPSDATA' then
				-- This one is slightly more complicated, because of the potential for a lot of data going to see if wee need to update first.'
				if upper(b_crew_only) = 'FALSE' then
					select count(*) into n_count from reports.TIODS_MP_HUND H ,reports.TIODS_MAINT_HIER MH --				
						where 1=1    and MH.CREW_TYP = s_crew_type  and h.lrm_key = mh.lrm_key   and h.mp between mh.beg_mp_no and mh.end_mp_no --
							--and (lpad(regexp_replace( dist, '[^[:digit:]]', null ),2,0) = s_district or crew = s_crew)  
							and h.data_updt_dt >= d_last_sync_date and rownum <5;
				else
					select count(*) into n_count from reports.TIODS_MP_HUND H ,reports.TIODS_MAINT_HIER MH --				
						where 1=1    and MH.CREW_TYP = s_crew_type  and h.lrm_key = mh.lrm_key   and h.mp between mh.beg_mp_no and mh.end_mp_no --
							--and (crew = s_crew)  
							and h.data_updt_dt >= d_last_sync_date 
							and rownum <5;
				end if;
				...
				if n_count = 0 then 
					...
					s_sql := q'[select h.lrm_key  ,regexp_replace( substr(h.lrm_key,1,5), '00$', null ) hwy_no ,h.rdwy_id   ,h.mlge_typ ,h.ovlp_cd ,h.MP ,h.rdwy_typ ,h.lat ,h.longtd   ,h.extrct_prc_dt ]';  --Cols
					...
					s_sql := s_sql || q'{ and (lpad(regexp_replace( dist, '[^[:digit:]]', null ),2,0) = '#dist#' or crew = '#crew#') and h.data_updt_dt >= '#date#' order by h.lrm_key, h.mp}'; --where 2
					--s_sql := s_sql || q'{ and  h.data_updt_dt >= '#date#' order by h.lrm_key, h.mp}'; --where 2
			...
			when 'TBLHWYLKUP' then
			/*
				This one is more complicated and needs a roll up from the reports schema.  
			*/
				select count(*) into n_count from reports.TIODS_MAINT_HIER MH where extrct_prc_dt >= d_last_sync_date;
				if n_count = 0 then 
					s_sql := 'select ''NO_DATA'' from dual';
				else
					s_sql := q'{select seg.lrm_key, seg.beg_mp_no, seg.end_mp_no
								,regexp_replace( substr(seg.lrm_key,1,5), '00$', null ) hwy_no,loc.IS_RTE, loc.US_RTE_1, loc.OR_RTE_1
								,hier.dist, hier.crew, hier.EA_NO
								from 
								reports.tiods_seg seg
								,reports.TIODS_RTE_HWY_LOC loc
								,reports.TIODS_MAINT_HIER hier
								where 1=1
								and hier.crew_typ ='SNCW'
								and seg.maint_uniq_id = hier.maint_uniq_id
								and seg.rte_uniq_id = loc.rte_uniq_id}';
					/*  Filtering Removed for this report.
								and (lpad(regexp_replace( dist, '[^[:digit:]]', null ),2,0) like '#dist#' or crew = '#crew#')}';
								
					s_sql := replace(s_sql,'#dist#',s_district);
					s_sql := replace(s_sql,'#crew#',s_crew);
					*/
					--need to fill it into the temp table before rolling it up
					delete from reports.x_signs_tblhwylkup_in;  --Just in case of a strange user scenario where the preserved GTT don't clear
					delete from reports.x_signs_tblhwylkup_out;
					
					
					EXECUTE IMMEDIATE 'insert into reports.x_signs_tblhwylkup_in '|| s_sql;
					
					reports.xodot_rollup_mp.rollup_mp('x_signs_tblhwylkup_in','x_signs_tblhwylkup_out');
					
					s_sql  := 'select * from reports.x_signs_tblhwylkup_out';
...
/* ************************************************************
	Object:		get_gen_dir_from_LRM
	Purpose:	retrieving the general direction of a hwy; mainly so that the correct XSP can be applied by the Broker
	Notes:		XPS is currently disabled in the broker
	Created:	2015.06.08	J.Mendoza
************************************************************* */
...
DROP PUBLIC SYNONYM xodot_signs_api;

CREATE OR REPLACE PUBLIC SYNONYM xodot_signs_api FOR TRANSINFO.xodot_signs_api;
