...
/*
	...
	UPDATE03:	Changes to Include start date so the copy command and certain changes work, 2015.04.10, JMM
	UPDATE04:	Added Comments to the private functions, 2015.06.05, JMM
*/
	...
	declare
		...
		*/
		...
		function get_domain_val (s_asset_name varchar2, b_use_old  boolean default false) return varchar2 is
/* ************************************************************
	Object:		get_domain_val
	Purpose:	Private FUnction to map the domain value with the asset 
	Notes:		Currently all are set to the same attribute
	Created:	2015.06.05	J.Mendoza
************************************************************* */
			s_ret_val varchar2(30) := 'N/A';
			begin
			if b_use_old = true then 
				case upper(s_asset_name)
		...
		function set_domain_rows return NM_INV_ATTRI_LOOKUP_ALL%rowtype is  r_return  NM_INV_ATTRI_LOOKUP_ALL%rowtype;
/* ************************************************************
	Object:		set_domain_rows
	Purpose:	Private Function to build a domain meaning from several asset fields
	Notes:		
	Created:	2015.06.05	J.Mendoza
************************************************************* */
	...
	begin
		...
		if Deleting then
			...
			gs_domain_val:= get_domain_val(:old.iit_inv_type,true);
			delete from NM_INV_ATTRI_LOOKUP_ALL 
				where ial_domain = gs_domain_name and ial_value = gs_domain_val and IAL_START_DATE = :old.IIT_START_DATE;
		...
		--removing the old domain first to stop start date conflicts
			gs_domain_name:= get_domain_name(:old.iit_inv_type);
			gs_domain_val:= get_domain_val(:old.iit_inv_type,true);
			delete from NM_INV_ATTRI_LOOKUP_ALL 
				where ial_domain = gs_domain_name and ial_value = gs_domain_val and IAL_START_DATE = :old.IIT_START_DATE;
			
			--since we removed the old one, we now make a new domain entry
