...
procedure snin_opps				( t_result 				out		xodot_signs_asset_op
	...
	exception when others then
		...
		xodot_signs_logging.log_exception(p_iit_ne_id, p_iit_ne_id ,s_LRM,n_MP,SQLCODE,'SNIN: ' || substr(SQLERRM,1,2000));
		nm3user.set_effective_date( trunc(sysdate));
		t_result := s_return_val;
end snin_opps;
...
PROCEDURE snsn_opps 			(t_result 						out		xodot_signs_asset_op
	...
	exception when others then
		...
		xodot_signs_logging.log_exception(p_iit_ne_id, p_iit_ne_id ,null,null,SQLCODE,substr(SQLERRM,1,2000));
		nm3user.set_effective_date( trunc(sysdate));
		t_result := s_return_val;
end snsn_opps;
			...
			*/
				...
				if n_count = 0 then 
					...
					s_sql := q'[select seg.lrm_key, seg.beg_mp_no, seg.end_mp_no
								...
								and (dist like '#dist#' or crew = '#crew#')]';
