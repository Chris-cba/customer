CREATE OR REPLACE PACKAGE BODY xact_portal AS
--
-----------------------------------------------------------------------------
--
--   SCCS Identifiers :-
--
--       sccsid           : @(#)xact_portal.pkb	1.1 03/14/05
--       Module Name      : xact_portal.pkb
...
--
   g_body_sccsid     CONSTANT  varchar2(2000) := '"@(#)xact_portal.pkb	1.1 03/14/05"';
--  g_body_sccsid is the SCCS ID for the package body
--
   g_package_name    CONSTANT  varchar2(30)   := 'xact_portal';
--
   c_this_module     CONSTANT  hig_modules.hmo_module%TYPE := 'XACTPORTAL0000';
...
FUNCTION get_xpl (p_xpl_xpc_id NUMBER
                 ,p_xpl_id     NUMBER
                 ) RETURN xact_portal_links%ROWTYPE;
--
-----------------------------------------------------------------------------
--
...
--
   l_rec_xpl xact_portal_links%ROWTYPE;
...
FUNCTION get_xpl (p_xpl_xpc_id NUMBER
                 ,p_xpl_id     NUMBER
                 ) RETURN xact_portal_links%ROWTYPE IS
   CURSOR cs_xpl IS
   SELECT *
    FROM  xact_portal_links
...
FUNCTION allowable_web_form (p_type   VARCHAR2
   ...
   WHERE  hmo_module      = c_module
    AND   hmo_application = xact_faq.c_xfaq;
BEGIN
   IF p_type = 'F'
    THEN
      l_retval := 'Y';
