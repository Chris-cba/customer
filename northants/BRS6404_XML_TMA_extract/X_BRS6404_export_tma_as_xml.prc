create or replace procedure X_BRS6404_export_tma_as_xml ( pi_file_dir IN varchar2, pi_URL IN VARCHAR2 )
--
-----------------------------------------------------------------------------
--
--   PVCS Identifiers :-
--
--       pvcsid                 : $Header:   //vm_latest/archives/customer/northants/BRS6404_XML_TMA_extract/X_BRS6404_export_tma_as_xml.prc-arc   1.0   Oct 10 2011 13:30:18   Ian.Turnbull  $
--       Module Name      : $Workfile:   X_BRS6404_export_tma_as_xml.prc  $
--       Date into PVCS   : $Date:   Oct 10 2011 13:30:18  $
--       Date fetched Out : $Modtime:   Oct 10 2011 09:19:16  $
...
-----------------------------------------------------------------------------
-- this procedure will create the TMA XML export file for Northants (NHSWM)
-- it require a view called X_V_BRS6404.
...
--  
  v_file_name := 'sdep' || TO_CHAR(sysdate,'YYYYMMDDHH24MISS') || '.xml';
  nm_debug.debug( 'BRS6404: directory: ' || pi_file_dir );
  nm_debug.debug( 'BRS6404: XML file_name: ' || v_file_name );
  begin
      fHandler := UTL_FILE.FOPEN(pi_file_dir, v_file_name, 'w');
  EXCEPTION
  WHEN utl_file.invalid_path THEN
     nm_debug.debug( 'BRS6404: Invalid path for file name'  ||  v_file_name );
     raise_application_error(-20000, 'Invalid path. Create directory or set UTL_FILE_DIR.');
  WHEN OTHERS THEN
    nm_debug.debug( 'BRS6404: Unexpected error opening file: ' ||SQLERRM||' -ERROR- '||SQLERRM);
    RAISE;
  END; 
  ...
  for rec in (select * from X_V_BRS6404 )
  loop
    ...
    UTL_FILE.PUT_LINE( fHandler, '<Description>' ||X_BRS6404_XML_SPECIAL_CHARS(rec.Description) || '</Description>' ); -- description
    ...
    if cur_noticeID%NOTFOUND THEN
       nm_debug.debug( 'BRS6404 error - Could not find notice for ' || rec.OrgEventReference || ' phase no ' || rec.PhaseNumber );
    end if;
    close cur_noticeID;  
    --   
    ...
    UTL_FILE.PUT_LINE( fHandler, '<OrganisationSectionName>' || X_BRS6404_XML_SPECIAL_CHARS(rec.OrgDistName) || '</OrganisationSectionName>');
    ...
    UTL_FILE.PUT_LINE( fHandler, '<LocationDescription>' ||X_BRS6404_XML_SPECIAL_CHARS(rec.LocationDescription) || '</LocationDescription> ' );
  ...
  nm_debug.debug( 'BRS6404: XML finished ' );
END;
/ 