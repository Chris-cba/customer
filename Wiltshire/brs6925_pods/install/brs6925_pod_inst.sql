...
--   PVCS Identifiers :-
--
--       pvcsid           : $Header:   //vm_latest/archives/customer/Wiltshire/brs6925_pods/install/brs6925_pod_inst.sql-arc   1.0   Dec 20 2011 10:45:34   Ian.Turnbull  $
--       Module Name      : $Workfile:   brs6925_pod_inst.sql  $
--       Date into PVCS   : $Date:   Dec 20 2011 10:45:34  $
--       Date fetched Out : $Modtime:   Dec 14 2011 11:05:44  $
...
BEGIN
   --
      IF USER IN ('SYS','SYSTEM')
       THEN
         RAISE_APPLICATION_ERROR(-20000,'You cannot install IM PODs as ' || USER);
      END IF;
END;      
/
...
prompt
prompt About to install Defect/Pem Link using exor base : &exor_base
prompt
accept ok_res prompt "OK to Continue with this setting ? (Y/N) "

select decode(upper('&ok_res'),'Y','&exor_base'||'brs6925_pods'||'&terminator'||
        'install'||'&terminator'||'brs6925_pod_install','exit') run_file
from dual
/
