...
*/
	...
	cursor c_brdg(s_brdge_id varchar2, s_bridge_suffix varchar2) is 
		select iit_type, iit_ne_id,IIT_CHR_ATTRIB26, IIT_CHR_ATTRIB27  from nm_inv_items
		where iit_inv_type = 'BRDG'
		and IIT_CHR_ATTRIB26 = s_brdge_id
		and nvl(iit_chr_attrib27,'none') = nvl(s_bridge_suffix, 'none')
		;
...
*/
   NM_DEBUG.PROC_START(G_PACKAGE_NAME,'load_on_element');
	b_has_rows := false;
	for r_row in c_brdg(p_rec.BRIDGE_ID,p_rec.BRIDGE_SUFFIX) loop
		b_has_rows := true;
