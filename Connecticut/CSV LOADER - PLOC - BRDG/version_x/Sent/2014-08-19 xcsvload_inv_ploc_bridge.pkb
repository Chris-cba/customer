...
*/
	G_PACKAGE_NAME    				CONSTANT  VARCHAR2(30)   	:= 'xcsvload_inv_ploc_bridge';
	...
	cursor c_brdg(s_brdge_id varchar2) is 
		...
		and trim(ltrim( IIT_CHR_ATTRIB26,'0') || nvl(iit_chr_attrib27,'')) in (
                            select cast(ltrim(myrow,'0') as varchar2(50)) myrow from   (SELECT EXTRACT(column_value,'/e/text()') myrow from
                            (select  s_brdge_id col1 from dual) x,
		...
		for r_row in c_brdg(y_row.myrow) loop
			b_has_rows := true;
				for t_row in c_get_ne_id_slk(r_row.iit_ne_id) loop
					...
					p_rec_old_tbl(p_rec_old_tbl.count).NE_UNIQUE			:= nm3net.get_ne_unique(t_row.mem_nm_ne_id_in);
					p_rec_old_tbl(p_rec_old_tbl.count).NE_NT_TYPE			:= p_rec.NE_NT_TYPE;
					p_rec_old_tbl(p_rec_old_tbl.count).BEGIN_MP				:= t_row.slk;
					p_rec_old_tbl(p_rec_old_tbl.count).END_MP				:= t_row.end_slk;
...
*/
   ...
   if p_rec.count >1 then  -- assuming only 2
		   --- START --------section to repeat
			C_END_MP   := NVL(P_REC(p_rec.last).END_MP,P_REC(p_rec.last).BEGIN_MP);
		   L_REC_NE := NM3GET.GET_NE (PI_NE_ID  => NM3NET.GET_NE_ID (P_REC(p_rec.last).NE_UNIQUE,p_rec(p_rec.last).NE_NT_TYPE));

		   
		   IF   NVL(P_REC(p_rec.last).BEGIN_MP,0)            = 0
			AND NVL(C_END_MP,L_REC_NE.NE_LENGTH) = L_REC_NE.NE_LENGTH
			THEN
			  L_BEGIN_MP := NULL;
			  L_END_MP   := NULL;
		   ELSE
			  L_BEGIN_MP := P_REC(p_rec.last).BEGIN_MP;
			  L_END_MP   := C_END_MP;
		   END IF;
