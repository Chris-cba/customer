...
-- ###############################################################
--
--  File           : 567.PLOC_LOADER_X.sql
--  Extracted from : ATLAS@ctdotsb.SGSPRDODB04
--  Extracted by   : ATLAS
--  At             : 21-AUG-2014 16:52:15
--
-- ###############################################################
--
...
BEGIN
--
   l_rec_nlf.nlf_unique           := 'PLOC_LOADER_X';
--
...
--
   add_nlfc ('ASSET_TYPE',nm3type.c_varchar,4,'Y',1,Null);
   add_nlfc ('ROAD_UNIQUE',nm3type.c_varchar,30,'Y',2,Null);
   add_nlfc ('NETWORK_TYPE',nm3type.c_varchar,4,'Y',3,Null);
   add_nlfc ('START_MP',nm3type.c_number,Null,'Y',4,Null);
   add_nlfc ('END_MP',nm3type.c_number,Null,'Y',5,Null);
   add_nlfc ('ADMIN_UNIT',nm3type.c_varchar,10,'Y',6,Null);
   add_nlfc ('START_DATE',nm3type.c_date,Null,'N',7,'DD-MON-YYYY');
   add_nlfc ('XSP',nm3type.c_varchar,4,'N',8,Null);
   add_nlfc ('PROJECT_NUMBER',nm3type.c_varchar,10,'Y',9,Null);
   add_nlfc ('PROJECT_TYPE',nm3type.c_varchar,10,'N',10,Null);
   add_nlfc ('PROJOECT_END_DATE',nm3type.c_varchar,4,'N',11,Null);
   add_nlfc ('PROJECT_DIRECTION',nm3type.c_varchar,2,'N',12,Null);
   add_nlfc ('PROJECT_SECTION',nm3type.c_varchar,50,'N',13,Null);
--
   l_rec_nlfd.nlfd_nld_id         := nm3get.get_nld (pi_nld_table_name => 'X_V_INV_ITEM_LOADER').nld_id;
   l_rec_nlfd.nlfd_seq            := 1;
   nm3ins.ins_nlfd (l_rec_nlfd);
--
   upd_nlcd ('IIT_NE_ID','X_CTDOT_PLOC_INV_LOAD.GET_IIT_ID_PROJ(PLOC_LOADER_X.PROJECT_NUMBER)');
   upd_nlcd ('IIT_INV_TYPE',''||CHR(39)||'PLOC'||CHR(39)||'');
   upd_nlcd ('IIT_PRIMARY_KEY',Null);
   upd_nlcd ('IIT_START_DATE','PLOC_LOADER_X.START_DATE');
   upd_nlcd ('IIT_DATE_CREATED',Null);
   upd_nlcd ('IIT_DATE_MODIFIED',Null);
   upd_nlcd ('IIT_CREATED_BY',Null);
   upd_nlcd ('IIT_MODIFIED_BY',Null);
   upd_nlcd ('IIT_ADMIN_UNIT','nm3get.get_nau(pi_nau_unit_code=>PLOC_LOADER_X.ADMIN_UNIT,pi_NAU_ADMIN_TYPE=>'||CHR(39)||'AST'||CHR(39)||').nau_admin_unit');
   ...
   upd_nlcd ('IIT_CHR_ATTRIB26','PLOC_LOADER_X.PROJECT_NUMBER');
   upd_nlcd ('IIT_CHR_ATTRIB27','PLOC_LOADER_X.PROJECT_TYPE');
   upd_nlcd ('IIT_CHR_ATTRIB28','PLOC_LOADER_X.PROJOECT_END_DATE');
   upd_nlcd ('IIT_CHR_ATTRIB29','PLOC_LOADER_X.PROJECT_DIRECTION');
   upd_nlcd ('IIT_CHR_ATTRIB30','PLOC_LOADER_X.PROJECT_SECTION');
   ...
   upd_nlcd ('IIT_X_SECT','PLOC_LOADER_X.XSP');
...
--
   l_rec_nlfd.nlfd_nld_id         := nm3get.get_nld (pi_nld_table_name => 'X_V_LOAD_INV_MEM_ON_ELEMENT_P').nld_id;
   l_rec_nlfd.nlfd_seq            := 2;
   nm3ins.ins_nlfd (l_rec_nlfd);
--
   upd_nlcd ('NE_UNIQUE','PLOC_LOADER_X.ROAD_UNIQUE');
   upd_nlcd ('NE_NT_TYPE','PLOC_LOADER_X.NETWORK_TYPE');
   upd_nlcd ('BEGIN_MP','PLOC_LOADER_X.START_MP');
   upd_nlcd ('END_MP','PLOC_LOADER_X.END_MP');
   upd_nlcd ('IIT_NE_ID','VIIL.IIT_NE_ID');
   upd_nlcd ('IIT_INV_TYPE',''||CHR(39)||'PLOC'||CHR(39)||'');
   upd_nlcd ('NM_START_DATE','PLOC_LOADER_X.START_DATE');
--
