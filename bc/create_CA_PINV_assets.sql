...
begin
  for c1rec in (select ne_id from 
                    nm_elements_all where ne_nt_type = 'ROAD' and ne_type = 'S' and ne_end_date is null
                    and ne_id not in (
                            select nm_ne_id_of from nm_members where nm_obj_type = 'CA'
                                    )
                 )
  loop
...
begin
  ...
  and ne_id not in (
                            select nm_ne_id_of from nm_members where nm_obj_type = 'PINV'
                                    )
                 )
  loop
    g_rec_ne := nm3get.get_ne_all (pi_ne_id => c1rec.ne_id);
