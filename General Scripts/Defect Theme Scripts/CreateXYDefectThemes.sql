...
--   PVCS Identifiers :-
--
--       pvcsid           : $Header:   //vm_latest/archives/customer/General Scripts/Defect Scripts/CreateXYDefectThemes.sql-arc   1.0   Jul 15 2010 14:32:20   Ian.Turnbull  $
--       Module Name      : $Workfile:   CreateXYDefectThemes.sql  $
...
---------------------------------------------------------------------------------------------------
--
-- Written by Aileen Heal to create the standandard defect XY themes
--
...
---------------------------------------------------------------------------------------------------
-- Spool to Logfile
define logfile1='CreateXYDefectsTheme&log_extension'
set pages 0
set lines 200
...
declare
  ...
  CURSOR drop_existing_defect_lyrs IS
      select * from nm_themes_all 
         where nth_feature_table in ('MAI_DEFECTS_XY_SDO', 
                                     'V_MAI_DEF_ACTIVITY_XY_SDO',
                                     'V_MAI_DEF_ACT_STA_XY_SDO', 
                                     'V_MAI_DEF_STATUS_XY_SDO');
--
--
begin
  select hop_value
  ...
  -- drop defect layers
  nm_debug.debug('Dropping XY Defect Layers');
  ...
  nm_debug.debug('Creating Defect Layers');
  NM3LAYER_TOOL.CREATE_DEFECT_LAYER(pi_theme_name    => 'DEFECTS',
                                    pi_asset_type    => 'DEFX',
                                    pi_asset_descr   => 'Defects XY',
                                    pi_x_column      => 'DEF_EASTING',
                                    pi_y_column      => 'DEF_NORTHING',
                                    pi_lr_ne_column  => NULL,
                                    pi_lr_st_chain   => NULL);
                                    
  nm_debug.debug('Created Defect Layers');
...
@create_sdo_themes.sql 'V_MAI_DEF_STATUS_XY_SDO' 'GEOLOC' 'DEFECTS BY STATUS' 'DEFECT_STATUS_CODE' 'V.EXOR.DEFECT.STATUS - V1.0'

-- create defects by CATEGORY sdo theme
@create_sdo_themes.sql 'V_MAI_DEF_ACT_STA_XY_SDO' 'GEOLOC' 'DEFECTS BY CATEGORY' 'DEFECT_PRIORITY' 'V.EXOR.DEFECT.CATEGORY - V1.0'
