...
--   PVCS Identifiers :-
--
--       pvcsid           : $Header:   //vm_latest/archives/customer/General Scripts/NSG Themes/Create_road_name_search.sql-arc   1.1   Jul 20 2010 14:35:58   Ian.Turnbull  $
--       Module Name      : $Workfile:   Create_road_name_search.sql  $
--       Date into PVCS   : $Date:   Jul 20 2010 14:35:58  $
--       Date fetched Out : $Modtime:   Jul 20 2010 14:17:28  $
...
--
-- The themes for Type 1 and 2 Streets must exist.
--
-- If an inv type of RNAM already exists then the creation of the ASSET metamodel is skipped.
--
------------------------------------------------------------------------------------------------------
...
BEGIN
   ...
   -- check is asset metamodel RNAM already exists
   select 1 
     into l_dummy
     from nm_inv_types 
    where nit_inv_type = 'RNAM';
   
   if l_dummy = 1 then
      dbms_output.PUT_LINE('RNAM asset type already exists skip creating' );
   else 
