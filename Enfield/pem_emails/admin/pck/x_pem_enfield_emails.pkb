CREATE OR REPLACE PACKAGE BODY x_pem_enfield_emails
AS
--
-----------------------------------------------------------------------------
--
--   PVCS Identifiers :-
--
--       pvcsid                 : $Header:   //vm_latest/archives/customer/Enfield/emails/x_pem_enfield_emails.pkb-arc   1.0   Feb 28 2011 11:53:48   Ian.Turnbull  $
--       Module Name      : $Workfile:   x_pem_enfield_emails.pkb  $
--       Date into PVCS   : $Date:   Feb 28 2011 11:53:48  $
--       Date fetched Out : $Modtime:   Feb 28 2011 11:52:18  $
...
PROCEDURE process_docs AS
	...
	CURSOR c1 IS
		SELECT a.doc_id, doc_user_id, doc_status_code, doc_dtp_code, doc_dcl_code ,doc_compl_type, doc_outcome, nmu_email_address, doc_compl_location, doc_descr, doc_compl_action, doc_compl_target, doc_compl_user_id, doc_compl_cpr_id,DOC_COMPL_INSURANCE_CLAIM,DOC_COMPL_POLICE_NOTIF_FLAG
...
PROCEDURE responsibility_email(	pi_doc_id					   IN NUMBER,
								...
								pi_DOC_COMPL_POLICE_NOTIF_FLAG IN VARCHAR2) IS
...
--
   ...
   select hct_id,hct_title,hct_first_name,hct_surname,hct_home_phone,hct_work_phone, hct_mobile_phone from hig_contacts
...
--
   ...
   select had_building_no,had_building_name, had_thoroughfare,had_post_town,had_postcode  from hig_address
   where had_id = (select hca_had_id from hig_contact_address
...
--
            ...
            IF get_user%FOUND THEN
               ...
               FETCH get_contact INTO l_hct_id, l_hct_title, l_hct_first_name, l_hct_surname, l_hct_home_phone, l_hct_work_phone, l_hct_mobile_phone;
               CLOSE get_contact;

               OPEN get_address (l_hct_id);
               FETCH get_address INTO l_had_building_no,l_had_building_name, l_had_thoroughfare,l_had_post_town,l_had_postcode;
               CLOSE get_address;

               IF c_mail_user%FOUND and l_status <> 'Complete' THEN
                  l_to (1).rcpt_id := l_nmu_rec.nmu_id;
                  l_to (1).rcpt_type := nm3mail.c_user;
                  l_subject  := 'NEW TEST Enquiry ' || pi_doc_id ||' has been assigned to you.  Priority = '||NVL(l_priority,'n/a');
                  l_msg (1)  := lfcr;
                  l_msg (2)  := 'Enquiry ' || pi_doc_id ||' has been assigned to '||l_nmu_rec.nmu_name||'            Status = ' || pi_doc_status_code|| ' ' || l_status;
                  l_msg (3)  := lfcr;
                  l_msg (4)  := 'Priority: '||NVL(l_priority,'n/a') ||'             Category: '||l_cat;
                  l_msg (5)  := lfcr;
                  l_msg (6)  := 'Class: '||l_class ||'                              Enquiry Type: '||l_type;
                  l_msg (7)  := lfcr;
                  l_msg (8)  := 'Potential Property Claim: '||pi_DOC_COMPL_POLICE_NOTIF_FLAG||'                     Potential Personal Claim: '||pi_DOC_COMPL_INSURANCE_CLAIM;
                  l_msg (9)  := lfcr;
                  l_msg (10) := 'Location: '||NVL(pi_doc_compl_location,'n/a');
                  l_msg (11) := lfcr;
                  l_msg (12) := 'Description: '||NVL(pi_doc_descr,'n/a');
                  l_msg (13) := lfcr;
                  l_msg (14) := 'Contact Details: '||l_hct_title||' '||l_hct_first_name||' '||l_hct_surname||' '||l_had_building_no||' '||l_had_building_name||' '||l_had_thoroughfare||' '||l_had_post_town||' '||l_had_postcode;
                  l_msg (15) := lfcr;
                  l_msg (16) := 'Tel Home: '||NVL(l_hct_home_phone,'n/a')||'   Tel Work: '||nvl(l_hct_work_phone,'n/a')||'   Tel Mobile: '||nvl(l_hct_mobile_phone,'n/a');
--
...
--
               ...
               ELSIF c_mail_user%FOUND and l_status = 'Complete'   THEN
                 NULL;
               ELSE
                  CLOSE c_mail_user;
               -- See if the user is actually a contact
                  ...
                  IF c_mail_user_contact%FOUND and l_status <> 'Complete' THEN
                     ...
                     l_subject := 'NEW TEST Enquiry ' || pi_doc_id ||' has been assigned to you.  Priority = '||NVL(l_priority,'n/a');
                     v_message := '';
                     v_message := v_message||lfcr||'Enquiry ' || pi_doc_id ||' has been assigned to '||l_hc_rec.hct_surname||'            Status = ' || pi_doc_status_code|| ' ' || l_status;
                     ...
                     v_message := v_message||lfcr;
                     v_message := v_message||lfcr||'Potential Property Claim: '||pi_DOC_COMPL_POLICE_NOTIF_FLAG||'                     Potential Personal Claim: '||pi_DOC_COMPL_INSURANCE_CLAIM;
                     v_message := v_message||lfcr;
                     ...
                     v_message := v_message||lfcr;
                     v_message := v_message||lfcr||'Tel Home: '||NVL(l_hct_home_phone,'n/a')||'   Tel Work: '||nvl(l_hct_work_phone,'n/a')||'   Tel Mobile: '||nvl(l_hct_mobile_phone,'n/a');
--
...
--
               ...
               ELSIF c_mail_user_contact%FOUND and l_status = 'Complete'   THEN
                 null;
