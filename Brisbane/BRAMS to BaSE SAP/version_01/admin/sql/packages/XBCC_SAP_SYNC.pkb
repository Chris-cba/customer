...
*/
	...
	procedure process_refcur  (d_start_date in  date, d_end_date in date, rc_recordset out sys_refcursor, n_brams_id in number default null, b_Only_Check_Cooridor in boolean default false) as
		...
		cursor c_corridor_ch(d_start_date  date, d_end_date date, s_brams_id varchar2) is
		...
		cursor c_inv_ch is
			...
			TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<ROW><e>'||REPLACE(col1,',','</e><e>')||'</e></ROW>'),'//e')))                        
         	...
         	, il as (SELECT EXTRACT(column_value,'/e/text()') myrow from (select   s_inv_type col1 from dual) x,
			TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<ROW><e>'||REPLACE(col1,',','</e><e>')||'</e></ROW>'),'//e'))))
			select mem.nm_ne_id_in ne_id from
				nm_members_all aloc
                ,nm_members_all mem
                ,XBCC_INV_IN_LIST i
				where 
		...
		cursor c_group_ch(d_start_date  date, d_end_date date, s_brams_id varchar2) is
		...
		with g as  (SELECT EXTRACT(column_value,'/e/text()') myrow from (select 'CBSC,CALL,KCON' col1 from dual) x,
                        TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<ROW><e>'||REPLACE(col1,',','</e><e>')||'</e></ROW>'),'//e')))
                        )
						, c as  (SELECT EXTRACT(column_value,'/e/text()') myrow from (select s_cor_type col1 from dual) x,
                        TABLE(XMLSEQUENCE(EXTRACT(XMLTYPE('<ROW><e>'||REPLACE(col1,',','</e><e>')||'</e></ROW>'),'//e')))
                        )
			,base1 as(
								SELECT /*+ materialize */ 
								...
								FROM 
									nm_inv_items_all i -- (select iit_ne_id from  nm_inv_items where 1=1 and iit_inv_type = INV_TYPE)) aloc
                                   -- (select * from nm_inv_items_all where iit_x_sect = 'XCS') i
									--,nm_members_all aloc
								...
								WHERE 1=1
									and ((i.iit_x_sect = 'XCS' and i.iit_inv_type = 'CBSC') OR (i.iit_x_sect = 'XK' and i.iit_inv_type = 'KCON') OR (i.iit_x_sect = 'XCM' and i.iit_inv_type = 'CALL') )
									and mem.nm_ne_id_in like n_brams_id
									AND mem.nm_obj_type in (select trim( cast(myrow as varchar2(25))) from c) 
									--AND aloc.nm_type = 'I'       15207963
				...
				from (
						select
							...
							,Indicator
						from (	select * from base1    									
								) 
						)
				group by mem_nm_ne_id_in, Condition --, DBTSMP, DB
...
procedure write_output_condition(n_ne_id number,b_write_med boolean, d_end_date date, r_elements nm_elements_all%rowtype ) as
	...
	begin  --process_refcur --------------------------------------------------- 
		...
		s_log_area := 'Main-1a'; 
		if n_brams_id is not null then
		...
		for r_row in c_corridor_ch(d_start_date , d_end_date ,  s_brams_id_chkd) loop
		...
		if b_Only_Check_Cooridor = false then
			...
			s_log_text := d_start_date ||', '|| d_end_date ||', '||  s_brams_id_chkd || ',GOLBAL: ' ||s_inv_type|| ', ' ||s_route_type;
			x_log_table.log_item(s_module, s_log_area, s_log_base_info , s_log_text );
			if s_brams_id_chkd <> '%' then 
				...
				s_log_text := d_start_date ||', '|| d_end_date ||', '||  s_brams_id_chkd || ',GOLBAL: ' ||s_group_type|| ', ' ||s_route_type;
				x_log_table.log_item(s_module, s_log_area, s_log_base_info , s_log_text );
				
				for r_row in c_group_ch(d_start_date , d_end_date ,  s_brams_id_chkd) loop
