...
/*
	...
	UPDATE06:	Changed Status check for corridors to Only check for a Delete/Insert during the provided dates, otherwise it is a CHANGE, 
				Forced provided BRAMS ID to always Report a CHANGE for all items				
				Added Status check for asset items, this was previously keyed form the corridor, 2014.09.26, JMM
...
*/
	...
	procedure process_refcur  (d_start_date in  date, d_end_date in date, rc_recordset out sys_refcursor, n_brams_id in number default null, b_Only_Check_Cooridor in boolean default false, b_log_to_x_log_information in boolean default false) as
		...
		s_indicator_asset 	varchar2(5) := null;
		...
		cursor c_get_owner(n_brams_id number, s_cor_type varchar2, d_end_date date) is
			...
			group by mem_nm_ne_id_in, i_asset_owner, indicator--, DBTSMP, DB
			order by slk
			;
		...
		cursor c_get_ward(n_brams_id number, s_cor_type varchar2, d_end_date date) is
			...
			group by mem_nm_ne_id_in, i_ne_descr, indicator --, DBTSMP, DB
			order by slk
			;
		...
		cursor c_get_suburb(n_brams_id number, s_cor_type varchar2, d_end_date date) is
			...
			group by mem_nm_ne_id_in, i_ne_descr, DBSMP, indicator
			order by slk
			;
...
------
		...
		function find_cor_indicator(n_ne_id number, d_start date, d_end date) return varchar2 as
			...
			begin
				...
				if s_brams_id_chkd <> '%' then return 'C'; end if; -- case when brams ID supplied
				
				if n_count = 1 then
					
					select * into r_nm_elements from nm_elements_all where ne_id = n_ne_id;   -- should always have 1
					
					if r_nm_elements.ne_end_date  is not  null  and (r_nm_elements.ne_date_modified between d_start and d_end) and trunc(r_nm_elements.ne_date_modified) <> d_start then
						return 'D';
					elsif r_nm_elements.ne_date_modified = r_nm_elements.ne_date_created and (r_nm_elements.ne_date_modified between d_start and d_end)  and trunc(r_nm_elements.ne_date_modified) <> d_start then
						return 'I';
					else 
						return 'C';
					end if;
		...
		procedure write_output_owner(n_ne_id number,b_write_med boolean, d_end_date date, r_elements nm_elements_all%rowtype ) as
			...
			for r_row in c_get_owner(n_ne_id,r_elements.ne_nt_type,d_end_date) loop
				...
				if s_indicator_cor = 'C' and s_brams_id_chkd = '%' then		
					s_indicator_asset := r_row.indicator;
				else 
					s_indicator_asset := s_indicator_cor;
				end if;
...
procedure write_output_condition(n_ne_id number,b_write_med boolean, d_end_date date, r_elements nm_elements_all%rowtype ) as
	...
	begin  --process_refcur --------------------------------------------------- 
		...
		if s_brams_id_chkd <> '%' then	
			insert into XBCC_CORR_LIST (ne_id) values (s_brams_id_chkd);
		end if;
		...
		for r_row in c_cor_list_cor loop
			s_log_area := 'Main-4'; 
			s_indicator_cor := find_cor_indicator(r_row.ne_id, d_start_date, d_end_date);
