...
Create or replace package body xBCC_Equip_Attr_Views as
/*
...
procedure p_process(s_primary_view in varchar2 default '%', d_start_date in date default sysdate, d_end_date in date default to_date('01019999','MMDDRRRR'), d_last_ran_before in date default to_date('01019999','MMDDRRRR')) as
	...
	procedure set_dates(d_start date, d_end date) is
		d_sd date;
		d_ed date;
		s_error_text varchar2(1000);
		e_error exception;
		begin

			if d_end < d_start then
				s_error_text := 'Start Date Must be less than the End Date';
				raise e_error;
			end if;
			if d_start is null then 
				d_sd := trunc(sysdate);
			else 
				d_sd := d_start;
			end if;
			if d_end = '01-JAN-9999' or d_end = null then d_ed := d_start; else d_ed := d_end; end if;
			
			XBCC_CONTEXT_DATES('START', to_char(d_sd, 'DD-MON-YYYY'));
			
			XBCC_CONTEXT_DATES('END', to_char(d_ed, 'DD-MON-YYYY'));
			
		exception when e_error then
			raise_application_error(-20001, s_error_text);
...
With 
ZDB_CHK as (
        select  * from xbcc_members_bt m, xbcc_elements_bt d
         where 1=1
         and ne_type = 'D'
...
,Base1 as 
    ...
    from     
        (SELECT 
            ...
            FROM 
            (select * from xbcc_members_bt where nm_ne_id_in in (select iit_ne_id from invitems)) aloc
            --,xbcc_members_bt aloc
            ,xbcc_members_bt mem           
        WHERE 1=1
...
select 
			...
			, (select * from xbcc_inv_items_bt) b
			, (select ne_id, ne_unique from xbcc_elements_bt) c
			where 
	...
	---********* start generate assets *********
	---********* start generate assets *********
	---********* start generate assets *********
	---********* start generate assets *********
	
	Begin  ---********* start generate assets *********
        set_dates(d_start_date,d_end_date);
		
		--Update Survey rolled up survey locations ----------
		
		delete from  xbcc_equip_attr_views_tt;
		
		insert into xbcc_equip_attr_views_tt select * from XBCC_EQUIP_ATTR_VIEWS_ESUR;
		------------
		...
		for r_row in c_assets	loop
			...
			s_date_col := q'[, TO_DATE (SYS_CONTEXT ('XBCC_DATES', 'START'), 'DD-MON-YYYY') Extract_Start_date,  TO_DATE (SYS_CONTEXT ('XBCC_DATES', 'END'), 'DD-MON-YYYY') Extract_end_date ]'; --'
			...
			s_sql_template_b := replace(s_sql_template_b,'##DATES##', s_date_col );
