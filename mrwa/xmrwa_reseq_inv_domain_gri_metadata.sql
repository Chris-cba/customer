...
--   SCCS Identifiers :-
--
--       sccsid           : @(#)xmrwa_reseq_inv_domain_gri_metadata.sql	1.1 03/15/05
--       Module Name      : xmrwa_reseq_inv_domain_gri_metadata.sql
--       Date into SCCS   : 05/03/15 00:46:03
--       Date fetched Out : 07/06/06 14:38:28
--       SCCS Version     : 1.1
--
--
...
--
   ...
   l_rec_grm.grm_pre_process :=            'DECLARE'
                                ...
                                ||CHR(10)||'BEGIN'
                                ||CHR(10)||'   -- @(#)xmrwa_reseq_inv_domain_gri_metadata.sql	1.1 03/15/05'
                                ||CHR(10)||'   SELECT ROWID the_rowid'
                                ||CHR(10)||'         ,(ROWNUM * :XNM0301_STEP)'
                                ||CHR(10)||'    BULK  COLLECT'
                                ||CHR(10)||'    INTO  lv_tab_rowid'
                                ||CHR(10)||'         ,lv_tab_seq'
                                ||CHR(10)||'    FROM (SELECT ROWID the_rowid'
                                ||CHR(10)||'           FROM  nm_inv_attri_lookup_all'
                                ||CHR(10)||'          WHERE  ial_domain = :XNM0301_DOM'
                                ||CHR(10)||'          ORDER BY ial_meaning'
                                ||CHR(10)||'         );'
                                ||CHR(10)||'   FORALL i IN 1..lv_tab_rowid.COUNT'
                                ||CHR(10)||'      UPDATE nm_inv_attri_lookup_all'
                                ||CHR(10)||'       SET   ial_seq = lv_tab_seq(i)'
                                ||CHR(10)||'      WHERE  rowid   = lv_tab_rowid(i);'
                                ||CHR(10)||'   COMMIT;'
                                ||CHR(10)||'END;';
--
   l_rec_hmo.hmo_module            := l_rec_grm.grm_module;
   l_rec_hmo.hmo_title             := 'Resequence Inventory Domain';
...
--
   l_rec_gp.gp_param           := 'XNM0301_DOM';
   l_rec_gp.gp_param_type      := 'CHAR';
   l_rec_gp.gp_table           := 'NM_INV_DOMAINS';
   l_rec_gp.gp_column          := 'ID_DOMAIN';
   l_rec_gp.gp_descr_column    := 'ID_TITLE';
   l_rec_gp.gp_shown_column    := 'ID_DOMAIN';
...
--
   l_rec_gp.gp_param           := 'XNM0301_STEP';
