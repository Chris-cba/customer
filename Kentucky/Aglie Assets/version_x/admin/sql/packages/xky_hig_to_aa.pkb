...
/*
	...
	UPDATE06:	"not found" code changed to include historic dates greater than the end date, 2015.01.15, JMM
*/
...
-----------------------------		Route Events		-----------------------------
		procedure process_route_events(s_test_route in varchar2 default  '%', b_debug_in in boolean default false, b_fill_work_flow_gaps in boolean default false) as
			...
			*/
			...
			r_err_for_output xaa_net_ref%rowtype;
			type t_net_ref is varray(100) of XAA_NET_REF%rowtype;
			...
			function route_changed (d_historic date, n_rt_name varchar2, n_beg number, n_end number) return number is
				...
				n_return := -1;
					...
					if n_count <> 1 then
						select count(*) into n_count from nm_elements_all where ne_nt_type = s_route_type and ne_type = 'G' and ne_unique = n_rt_name 
							and (trunc(ne_start_date) <= trunc(d_historic) AND NVL (ne_end_date, TO_DATE ('99991231', 'YYYYMMDD')) >  trunc(d_historic));
						if n_count = 0 then
							return 1;
						else
							--return 5;
				...
				*/
				begin
					...
					select count(*) into n_count from nm_elements_all where ne_nt_type = s_route_type and ne_type = 'G' 
						...
						;
					if b_debug = true and b_dbms_debug_me = true  then 
			...
			function get_old_loc_list(n_old_ne_id number, n_original_datum_offset number default null, b_is_start boolean default false, b_is_also_end boolean default false , b_is_single_point boolean default false ) return typ_neh_table is
...
************************************************************* */
				...
				begin
					...
					while n_count <> 0 
					loop
						--Checking the cardinality of the route datum if this is a start or end point so that we can sort correctly when "dropping" segments or applying offsets
						if n_new_offset is null then
							n_temp := 1;
						else
							if b_debug = true and b_dbms_debug_me = true then 
								dbms_output.put_line('-------------Checking Cardinality n_ne_id :  ' || to_char(n_ne_id) || ' n_old_rt_id: ' || to_char(n_old_rt_id) || '---');
							end if;
							
							--Dropping Route Check in case the Route Changes
							--select max(nvl(NM_CARDINALITY,1)) into n_card from nm_members_all where nm_ne_id_of in (n_ne_id) and nm_obj_type = 'RT' and nm_ne_id_in = n_old_rt_id ;
							select max(nvl(NM_CARDINALITY,1)) into n_card from nm_members_all where nm_ne_id_of in (n_ne_id) and nm_obj_type = 'RT';
						end if;
						...
						if n_card = 1 then 
							...
							for r_row in (select * from nm_element_history where neh_ne_id_old = n_ne_id order by neh_ne_id_new desc) loop
								a_neh_table.extend(1);
								a_neh_table(a_neh_table.count) := r_row;
							end loop;
						end if;
							...
							if b_debug = true and b_dbms_debug_me = true then 
								dbms_output.put_line('-------------Operation on n_ne_id :  ' || to_char(n_ne_id) || ' is '|| a_neh_table(a_neh_table.count).neh_operation ||  ' Count is: ' || a_neh_table.count || '---');
							end if;
						...
						if a_neh_table(a_neh_table.count).neh_ne_id_new = a_neh_table(a_neh_table.count).neh_ne_id_old OR a_neh_table(a_neh_table.count).neh_operation = 'S' then
							...
							n_ne_id := a_neh_table(a_neh_table.count).neh_ne_id_new;
								...
								--
								---- Checking to see if we need to adjust the start or end offset because of a merge or a recalibrate
								--
							...
							if  a_neh_table(a_neh_table.count).neh_operation = 'M' and n_new_offset is not null then 
								...
								begin
									...
									if n_card = 1 then
										...
										select  extra_len  into n_temp from (
											select b.*, 
												...
												from 
												(select a.*, 
													case when ne_id = ne_id_old and 	ne_no_end = lead(ne_no_start) over (order by ne_no_start  ) then 2 
														 when ne_id = ne_id_old and  	ne_no_start = lag(ne_no_end) over (order by ne_no_start  ) then 1
														 when ne_id = ne_id_old and 	ne_no_end = lag(ne_no_start) over (order by ne_no_start  ) then 2 
														 when ne_id = ne_id_old and  	ne_no_start = lead(ne_no_end) over (order by ne_no_start  ) then 1
														else 0 end pos      
												 from nm_elements_all a where ne_id in (select neh_ne_id_old from nm_element_history where neh_ne_id_new = ne_id_new and neh_operation = 'M')
												 order by ne_no_start) b
												 )
									...
									--lets see if we need to add this length to the offset
									/*select r_num into n_temp_b from(
										select rownum r_num, a.* from nm_element_history a where neh_ne_id_new = ne_id_new and neh_operation = 'M' order by neh_id
										) where neh_ne_id_old = ne_id_old;
									*/
									select r_num into n_temp_b from(
										select rownum r_num, b.* from (											
											select  b.* from 
											nm_members_all b
											,	(SELECT NM_NE_ID_OF, MAX(NM_date_created) NM_date_created from nm_members_all
													where  nm_obj_type = s_route_type and nm_ne_id_of in (select neh_ne_id_old from nm_element_history a 
																	where neh_ne_id_new = ne_id_new and neh_operation = 'M') group by NM_NE_ID_OF
												) c
											where b.nm_ne_id_of = c.nm_ne_id_of
											and b.nm_date_created = c.nm_date_created
											and nm_obj_type = s_route_type order by nm_slk
											) b
										) where nm_ne_id_of = ne_id_old;
										
									if b_debug = true and b_dbms_debug_me = true then 
										dbms_output.put_line('-------------**Merging n_card :  ' || to_char(n_card) || ' n_temp_b: ' || to_char(n_temp_b) || ' **');
									end if;
									
										
									if b_is_start= true and n_temp_b = 2 and n_card = 1 then 
									
										if b_debug = true and b_dbms_debug_me = true then 
											dbms_output.put_line('-------------**Merge Occurred Adding to start :  ' || to_char(n_temp) || ' New Offset is: ' || to_char(n_new_offset + n_temp) || ' **');
										end if;
										
										n_new_offset := n_new_offset + n_temp;
										
									elsif b_is_start = true and b_is_also_end = true and n_card = 1 and n_temp_b = 1 then
										if b_debug = true and b_dbms_debug_me = true then 
											dbms_output.put_line('-------------**Merge Occurred Adding to start :  ' || to_char(n_temp) || ' New Offset is: ' || to_char(n_new_offset + n_temp) || ' **');
										end if;
									...
									elsif b_is_start= false and n_card = 1 and n_temp_b = 2 then 									
										if b_debug = true and b_dbms_debug_me = true then 
											dbms_output.put_line('-------------**Merge Occurred Adding to end :  ' || to_char(n_temp) || ' New Offset is: ' || to_char(n_new_offset + n_temp) || ' **');
										end if;
										
										n_new_offset := n_new_offset + n_temp;
										
									-- Now reverse Cardinality 
									
									elsif b_is_start= true and n_temp_b = 2 and n_card = -1 then 
										if b_debug = true and b_dbms_debug_me = true then 
											dbms_output.put_line('-------------**Merge Occurred Adding to start :  ' || to_char(n_temp) || ' New Offset is: ' || to_char(n_new_offset + n_temp) || ' **');
										end if;
									...
									elsif b_is_start= false and n_temp_b = 2 and n_card = -1 then 
										if b_debug = true and b_dbms_debug_me = true then 
											dbms_output.put_line('-------------**Merge Occurred Adding to end :  ' || to_char(n_temp) || ' New Offset is: ' || to_char(n_new_offset + n_temp) || ' **');
										end if;
										...
										if b_debug = true and b_dbms_debug_me = true then 
											dbms_output.put_line('-------------**Merge Occurred No change :  ' || to_char(n_new_offset) ||  ' **');
										end if;
									end if;
								...
								if b_debug = true and b_dbms_debug_me = true then 
										dbms_output.put_line('---Current Offset :  ' || to_char(n_new_offset));
								end if;
							
							elsif a_neh_table(a_neh_table.count).neh_operation in ( 'R') and n_new_offset is not null then 
								...
								select nvl(slk_dif,0) slk_dif  into n_temp from (
									...
									if b_debug = true and b_dbms_debug_me = true then 
										dbms_output.put_line('-------------**Reclassify Occurred Adding to start :  ' || to_char(n_temp) || ' New Offset is: ' || to_char(n_new_offset - n_temp) || ' **');
									end if;

									--If the change SLK was moved back, I have no choice but to ignore it since I might lose the starting SLK and overshoot the datum, or if the change 
									--was bigger than the the original offset I have to ignore it.
									if n_temp < 0  then 
										n_temp :=0; 
									elsif n_temp > n_new_offset then
										n_temp := n_new_offset;
									end if;
							...
							elsif n_new_offset is not null then  -- lets check for Recalibrate
								...
								select count(*) into n_count from nm_element_history where neh_ne_id_new = ne_id_new and neh_operation = 'B';
								if n_count =1 then 
									-- In this situation if the length shortened I need to remove it from the offset or the SLK will be off.
									select max(neh_new_ne_length) - min(neh_old_ne_length) into n_temp from nm_element_history where neh_ne_id_new = ne_id_new and neh_operation = 'B';
									
										if b_debug = true and b_dbms_debug_me = true then 
											dbms_output.put_line('-------------**B Occurred Removing if negative  :  ' || to_char(n_temp) || ' New Offset is: ' || to_char(n_new_offset - n_temp) || ' **');
										end if;

										--If the change SLK was moved back, I have no choice but to ignore it since I might lose the starting SLK and overshoot the datum, or if the change 
										--was bigger than the the original offset I have to ignore it.
										if n_temp > 0 or abs(n_temp) > n_new_offset then 
											n_temp :=0; 
										end if;
						...
						for xx in a_neh_table.first..a_neh_table.last loop
							if a_neh_table(xx).neh_operation = 'S' then
								...
								if  n_new_offset is not null then 
									...
									select max(nvl(NM_CARDINALITY,1)) into n_card from nm_members_all where nm_ne_id_of in (ne_id_old) and nm_obj_type = 'RT';
									
									if b_debug = true and b_dbms_debug_me = true  then
										dbms_output.put_line('N_CARD = ' || n_card || ', ' || ' n_ne_id = ' ||ne_id_old || ', ' || 'n_old_rt_id = ' || n_old_rt_id );
									end if;
									
									--using n_temp_b for single point check
									n_temp_b := 0;  
									if b_is_single_point then n_temp_b :=1; end if;
									
									if b_debug = true and b_dbms_debug_me = true  then
										dbms_output.put_line('b_is_single_point (1=true) = ' || n_temp_b);
									end if;
									
									if b_is_start = true  and n_card= 1 then
										select s_keep into n_temp from 
												(select b.*,
													case when pos = 1 and n_new_offset <= ne_length then 0
														 when pos =2 and n_new_offset <= lag(ne_length) over (order by pos) and n_temp_b = 0 then 0 -- since this is a start datum, we keep the second
									...
									elsif b_is_start = true  and n_card= -1 then
											select s_keep into n_temp from 
												(select b.*,
													case when pos = 1 and n_new_offset <= ne_length then 0
														 when pos =2 and n_new_offset <= lag(ne_length) over (order by pos) then 0 -- since this is a start datum, we keep the second
														 when pos =2 and n_new_offset > lag(ne_length) over (order by pos) then  n_new_offset - lag(ne_length) over (order by pos)
													else -888
													end  s_keep 	
												from 
												(
														select a.*, 
															case when  	ne_no_end = lead(ne_no_start) over (order by ne_no_start ) then 2 
																 when   	ne_no_start = lag(ne_no_end) over (order by ne_no_start ) then 1
																 when  	ne_no_end = lag(ne_no_start) over (order by ne_no_start ) then 2 
																 when   	ne_no_start = lead(ne_no_end) over (order by ne_no_start ) then 1
																else 0 end pos      
														 from nm_elements_all a where ne_id in (select neh_ne_id_new from nm_element_history where neh_ne_id_old = ne_id_old and neh_operation = 'S' )
														 order by ne_no_start
														 ) b)
														where ne_id = ne_id_new
														order by pos;
									elsif  b_is_start = false  and n_card= 1 then--it is an end
									
										if b_debug = true and b_dbms_debug_me = true  then
											dbms_output.put_line('B_is_start = F, n_card = ' || n_card);
										end if;
											select s_keep into n_temp from 
												(select 
												case when pos =1 and n_new_offset <= ne_length and n_card = 1 then 0
													 when pos =2 and n_new_offset <= lag(ne_length) over (order by pos) and n_card = 1 then -999
													 when pos =2 and n_new_offset > lag(ne_length) over (order by pos) and n_card = 1 then  n_new_offset - lag(ne_length) over (order by pos)
													 when pos =1 and n_new_offset <= ne_length and n_card = -1 then 0
													 when pos =1 and n_new_offset >= ne_length and n_card = -1 then -999
													 when pos =2 and n_new_offset > lag(ne_length) over (order by pos) and n_card = -1 then  n_new_offset - lag(ne_length) over (order by pos)
												else -888
												end  s_keep 
												, b.*
										...
										if b_debug = true and b_dbms_debug_me = true  then
											...
											select s_keep into n_temp from 
												...
												case when pos =1 and n_new_offset <= ne_length and n_card = 1 then 0
													 when pos =2 and n_new_offset <= lag(ne_length) over (order by pos) and n_card = 1 then -999
													 when pos =2 and n_new_offset > lag(ne_length) over (order by pos) and n_card = 1 then  n_new_offset - lag(ne_length) over (order by pos)
													 when pos =1 and n_card = -1 and n_new_offset > lead(ne_length) over (order by pos) and n_card = -1 then  n_new_offset - lead(ne_length) over (order by pos)
													 when pos =1 and lead(n_new_offset) over (order by pos )  >= ne_length and n_card = -1 then -999
													 when pos =2 and n_new_offset <= ne_length and n_card = -1 then 0
												else -888
												end  s_keep 
												, b.*
												from 
								...
								elsif b_log_spilt= false and b_is_start = true then
									-------------------
									null;
									/*
									select nvl(ne_length,0) into n_temp from nm_elements_all where ne_id = a_neh_table(xx).neh_ne_id_new;
									
									if b_debug = true and b_dbms_debug_me = true then 
										dbms_output.put_line('-------------**Spilt Occurred Discarding :  ' || to_char(n_temp) || ' New Offset is: ' || to_char(n_new_offset - n_temp) || ' **');
									end if;
						...
						if spilt_neh_table.count <> 0 then 
							...
							for yy in spilt_neh_table.first..spilt_neh_table.last loop
								...
								if b_debug = true and b_dbms_debug_me = true  then 
									...
									if b_is_also_end = true then s_temp := 'T'; else s_temp := 'F'; end if;
									dbms_output.put_line('Sending Spilt to process: ' || spilt_neh_table(yy).neh_ne_id_new || ', Spilt Number: ' || yy || ', b_is_also_end: ' || s_temp );
								end if;
								
								--n_temp := 0;
								--if spilt_neh_table(yy).neh_ne_id_new = 8461611 then n_temp := 1; end if;
								...
								if b_is_start = false and yy = spilt_neh_table.first and spilt_neh_table.count = 2 then -- We the spilt is at the end and any more splits from this half  should not be handled as an end point.
									process_spilt_neh_table := get_old_loc_list(spilt_neh_table(yy).neh_ne_id_new, null, b_is_start,b_is_single_point);
								elsif b_is_start = true and yy = spilt_neh_table.last and spilt_neh_table.count = 2 and b_is_also_end =false then -- We the spilt is at the start and any more splits from this half  should not be handled as an start point.
									process_spilt_neh_table := get_old_loc_list(spilt_neh_table(yy).neh_ne_id_new, null, b_is_start,b_is_single_point);
								else
									process_spilt_neh_table := get_old_loc_list(spilt_neh_table(yy).neh_ne_id_new, n_new_offset, b_is_start,b_is_also_end,b_is_single_point);
			...
			*/
			...
			/* ************************************************************
				Object:		over_lap_test
				Purpose:	To test for any overlaps in xaa_route_temp and flag a manual verification message
				Notes:		
				Created:	2015.09.04	J.Mendoza
			************************************************************* */
			
			function over_lap_test return number is
				n_ov_code number;
				begin
					select nvl(min(c_test),5) into n_ov_code from (
						select distinct
						ne_id_in
						,ne_id_of
						,slk
						,end_slk
						, lead (slk) over (partition by ne_id_in order by slk) t1
						, lag(end_slk) over (partition by ne_id_in order by slk) t2
						, case when lead (slk) over (partition by ne_id_in order by slk) is null or    lead (slk) over (partition by ne_id_in order by slk) >= end_slk then 0
						  when lag(end_slk) over (partition by ne_id_in order by slk) is null or lag(end_slk) over (partition by ne_id_in order by slk) <= slk then 1
						  else -1
						  end c_test
						from xaa_route_temp_sql a
		...
		begin 	-------------------------------------------------------BEGIN----------------------------------------------------------
			...
			for r_input in c_route_list loop
				...
				begin
					if b_debug= true then 
						dbms_output.put_line('[-0] : ' || systimestamp);
					end if;
					...
					*/
					...
					if b_debug= true then 
						dbms_output.put_line('[00] : ' || systimestamp);
					end if;
					...
					if b_debug= true then 
						null;
						--dbms_output.put_line('[01] : ' || systimestamp);
					end if;
					...
					case 
						...
						when n_code = 3  then  --Date changed but item is still there
							...
							if b_debug= true then 
								null;
								--dbms_output.put_line('[02] : ' || systimestamp);
							end if;
							...
							if b_debug = true then
								s_log_base_info := 'Informational = Sending to c_nm_mem_effective_rt(n_old_rt_id,r_start_datum.datum_start_slk, r_end_datum.datum_end_slk, r_input.historic_date );';
								s_log_text :=n_old_rt_id|| ', ' ||r_start_datum.datum_start_slk|| ', ' || r_end_datum.datum_end_slk|| ', ' || r_input.historic_date ;
								x_log_table.log_item(s_module, s_log_area, s_log_base_info , s_log_text );
							end if;
							...
							if b_debug= true then 
								null;
								--dbms_output.put_line('[03] : ' || systimestamp);
							end if;
							
							if b_debug = true then
								s_log_base_info := 'Informational';
								s_log_text :='n_code =' || n_code || ' r_nm_members_all.count: ' || r_nm_members_all.count ;
								x_log_table.log_item(s_module, s_log_area, s_log_base_info , s_log_text );
							end if;
							...
							if r_nm_members_all.count = 0 then
								...
								if b_debug= true then 
									null;
									--dbms_output.put_line('[04] : ' || systimestamp);
								end if;								
								...
								for xx in r_nm_members_all.first..r_nm_members_all.last loop
									...
									--datum_start_slk number(10,3),datum_end_slk number(10,3), datum_offset number(10,3));
										if r_nm_members_all(xx).nm_ne_id_of = r_start_datum.old_ne_id_of then
											if 	 r_input.offset_from = r_input.offset_to then
												b_temp := true;    -- This in case the start and end offset share the same datum
												r_old_loc := get_old_loc_list(r_nm_members_all(xx).nm_ne_id_of,r_end_datum.datum_offset, b_temp, false,true);
											else
										...
										if r_nm_members_all(xx).nm_ne_id_of = r_end_datum.old_ne_id_of then
											b_temp := false;
											if r_end_datum.old_ne_id_of = r_start_datum.old_ne_id_of and r_input.offset_from <> r_input.offset_to then
												b_temp := true;    -- This in case the start and end offset share the same datum
												r_old_loc := get_old_loc_list(r_nm_members_all(xx).nm_ne_id_of,r_end_datum.datum_offset, b_temp, true);
											elsif 	r_end_datum.old_ne_id_of = r_start_datum.old_ne_id_of and r_input.offset_from = r_input.offset_to then
												b_temp := true;    -- This in case the start and end offset share the same datum
												r_old_loc := get_old_loc_list(r_nm_members_all(xx).nm_ne_id_of,r_end_datum.datum_offset, b_temp, false,true);
											else
												r_old_loc := get_old_loc_list(r_nm_members_all(xx).nm_ne_id_of,r_end_datum.datum_offset, b_temp, false);
											end if;
											if r_old_loc.count <> 0 then
												begin -- numeric check 
								...
								if b_debug= true then 
									--dbms_output.put_line('[05] : ' || systimestamp);
									null;
								end if;							
						...
						if b_debug = true then
							...
							if b_debug= true then 
								--dbms_output.put_line('[06] : ' || systimestamp);
								null;
							end if;
								...
								if b_debug= true then 
									--dbms_output.put_line('[07] : ' || systimestamp);
									null;
								end if;									
									...
									for x_row in c_chk_cont loop										
										...
										-- section is closed
												...
												if x_row.start_slk > x_row.end_slk or over_lap_test = -1  then
													r_output(n_idx).process_msg			:= 'Error in Processing, Manual Verification is Necessary'; --r_input.process_msg;
												else
													r_output(n_idx).process_msg		:= 'Route Location Closed'; --r_input.process_msg;
												end if;
										...
										if n_idx >= n_idx_flush_threshold then  -- see if the array is getting too big				
											flush_array;				
										end if;	
									...
									if b_debug = true then
										s_log_base_info := 'Checking c_chk_cont_current: ';									
										s_log_text := ' ';
									...
									for x_row in c_chk_cont_current loop
										if b_debug= true then 
											--dbms_output.put_line('[08] : ' || systimestamp);
											null;
										end if;
										...
										if b_debug= true then 
											--dbms_output.put_line('[A8] : ' || systimestamp);
											null;
										end if;
								end if;
						...
						when n_code = 5 then -- route name was end dated,
							if b_debug= true then 
								dbms_output.put_line('[09] : ' || systimestamp);
							end if;
									...
									if r_nm_members_all.count = 0 then
										...
										for xx in r_nm_members_all.first..r_nm_members_all.last loop
											...
											if r_nm_members_all(xx).nm_ne_id_of = r_start_datum.old_ne_id_of or r_nm_members_all(xx).nm_ne_id_of = r_end_datum.old_ne_id_of then
												...
												--given_slk number(10,3), 
												--datum_start_slk number(10,3),datum_end_slk number(10,3), datum_offset number(10,3));
										if r_nm_members_all(xx).nm_ne_id_of = r_start_datum.old_ne_id_of and xx = r_nm_members_all.first then
											r_old_loc := get_old_loc_list(r_nm_members_all(xx).nm_ne_id_of,r_start_datum.datum_offset,true);
											if r_old_loc.count <> 0 then 
												begin -- numeric check
													n_temp := to_number(r_old_loc(r_old_loc.last).neh_descr);
						...
						when n_code = 7 then -- point asset
							...
							*/
							if b_debug= true then 
								dbms_output.put_line('[11] : ' || systimestamp);
							end if;
					...
					if b_debug= true then 
						dbms_output.put_line('[12] : ' || systimestamp);
					end if;
					...
					if b_debug= true then 
						dbms_output.put_line('[13] : ' || systimestamp);
					end if;
				exception when others then
					err_num := SQLCODE;
					err_msg := SUBSTR(SQLERRM, 1, 200);
					x_log_table.log_item(s_module, 'R_Input_Loop', 'Unhandled Error_1', err_num || ' - ' || err_msg);
					x_log_table.log_item(s_module, 'R_Input_Loop', 'Unhandled Error_2', dbms_utility.FORMAT_ERROR_BACKTRACE);
					x_log_table.log_item(s_module, 'R_Input_Loop', 'Unhandled Error_3', err_num || ' - ITEM: ' ||  r_input.loc_ident);
					
					
					
											r_err_for_output.historic_date 		:= r_input.historic_date;
											r_err_for_output.loc_ident 			:= r_input.loc_ident;
											r_err_for_output.route_name			:= r_input.route_name;
											r_err_for_output.offset_from		:= r_input.offset_from;
											r_err_for_output.offset_to			:= r_input.offset_to;
											r_err_for_output.source_table		:= r_input.source_table; --upper('xaa_elements_all'); --r_input.source_table;
											r_err_for_output.new_route_name 	:= null; --r_input.new_route_name;
											r_err_for_output.new_date 			:= r_input.historic_date;
											r_err_for_output.new_offset_from	:= null;
											r_err_for_output.new_offset_to		:= null;
											r_err_for_output.process_msg		:= 'UNEXPECTED ERROR WITH ROUTE AND OFFSET'; --r_input.process_msg;
					
					insert into xaa_net_ref values r_err_for_output;
					commit;
					--RAISE; 
				end;
			...
			if b_debug= true then 
				dbms_output.put_line('[14] : ' || systimestamp);
			end if;
			...
			if b_debug= true then 
				dbms_output.put_line('[15] : ' || systimestamp);
			end if;
