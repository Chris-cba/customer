...
/*		
		function get_old_loc_list(n_old_ne_id number ) return typ_neh_table is
				...
				begin
					...
					dbms_output.put_line('---------------Starting Get Old List for: ' || n_old_ne_id);
					while n_count <> 0 
					loop
						...
						if a_neh_table(a_neh_table.count).neh_ne_id_new = a_neh_table(a_neh_table.count).neh_ne_id_old OR a_neh_table(a_neh_table.count).neh_operation = 'S' then
							...
							if  a_neh_table(a_neh_table.count).neh_operation = 'M' and (n_old_ne_id = r_start_datum.old_ne_id_of or n_old_ne_id = r_end_datum.old_ne_id_of then)
								--
					...
					if a_neh_table.count <> 0 then 
						for xx in a_neh_table.first..a_neh_table.last loop
							if a_neh_table(xx).neh_operation = 'S' then
								...
								if n_old_ne_id = r_start_datum.old_ne_id_of or n_old_ne_id = r_end_datum.old_ne_id_of then
									-- If there are spilts it is always the last 2 items in the list so last - 1 shoudl always be the frist spilt
									--info: type typ_start_end_datum is record(old_ne_id_of number(10), new_ne_id_of number(10), given_slk number(10,3), datum_start_slk number(10,3),datum_end_slk number(10,3));
									if a_neh_table.last -1 = xx and n_old_ne_id = r_start_datum.old_ne_id_of then
										select count(*) into n_count from nm_members_all a where a.nm_obj_type = 'RT' and  a.nm_ne_id_of =a_neh_table(xx).neh_ne_id_new
											and a.nm_start_date = (select max(nm_start_date) sd from nm_members_all b where b.nm_obj_type = a.nm_obj_type and b.nm_ne_id_of = a.nm_ne_id_of)
											and r_start_datum.given_slk  between nm_slk and nm_end_slk;
											
										if n_count <> 1 then 
											b_log_spilt := false;
											--we need to change the offset slk or the adjusting routine will be incorrect
											
											 select r_start_datum.datum_start_slk +(nm_end_slk - nm_slk) into r_start_datum.datum_start_slk from nm_members_all a where a.nm_obj_type = 'RT' and  a.nm_ne_id_of =a_neh_table(xx).neh_ne_id_new
																			and a.nm_start_date = (select max(nm_start_date) sd from nm_members_all b where b.nm_obj_type = a.nm_obj_type and b.nm_ne_id_of = a.nm_ne_id_of)
																			;
										end if;	
									end if;
									if a_neh_table.last = xx and n_old_ne_id = r_end_datum.old_ne_id_of then 
										select count(*) into n_count from nm_members_all a where a.nm_obj_type = 'RT' and  a.nm_ne_id_of =a_neh_table(xx).neh_ne_id_new
											and a.nm_start_date = (select max(nm_start_date) sd from nm_members_all b where b.nm_obj_type = a.nm_obj_type and b.nm_ne_id_of = a.nm_ne_id_of)
											and r_end_datum.given_slk  between nm_slk and nm_end_slk;
										
										if n_count <> 1 then 
											b_log_spilt := false;
											--we need to change the offset slk or the adjusting routine will be incorrect
											
											 select r_end_datum.datum_start_slk -(nm_end_slk - nm_slk) into r_end_datum.datum_end_slk from nm_members_all a where a.nm_obj_type = 'RT' and  a.nm_ne_id_of =a_neh_table(xx).neh_ne_id_new
																			and a.nm_start_date = (select max(nm_start_date) sd from nm_members_all b where b.nm_obj_type = a.nm_obj_type and b.nm_ne_id_of = a.nm_ne_id_of)
																			;
						...
						if spilt_neh_table.count <> 0 then 
							...
							for yy in spilt_neh_table.first..spilt_neh_table.last loop
								...
								--IF this spilt is the first part of the datum then temporally change the global first datum, in case it it spilts again
								if (n_old_ne_id = r_start_datum.old_ne_id_of AND YY = spilt_neh_table.FIRST) or (n_old_ne_id = r_end_datum.old_ne_id_of AND YY = spilt_neh_table.last) then
									if n_old_ne_id = r_start_datum.old_ne_id_of AND YY = spilt_neh_table.FIRST then
										r_start_datum.old_ne_id_of := spilt_neh_table(yy).neh_ne_id_new;
										process_spilt_neh_table := get_old_loc_list(spilt_neh_table(yy).neh_ne_id_new);
										r_start_datum.old_ne_id_of := n_old_ne_id;
									end if;
									if n_old_ne_id = r_end_datum.old_ne_id_of AND YY = spilt_neh_table.last then  -- doing the ends
										r_end_datum.old_ne_id_of := spilt_neh_table(yy).neh_ne_id_new;
										process_spilt_neh_table := get_old_loc_list(spilt_neh_table(yy).neh_ne_id_new);
										r_end_datum.old_ne_id_of := n_old_ne_id;
									end if;
						...
						dbms_output.put_line('    Final History for ' || n_old_ne_id || ' With a_neh_table.count of: ' || a_neh_table.count);
						for xx in a_neh_table.first .. a_neh_table.last  loop					
							...
							if n_count = 0 then -- if there is no history or links to itself then we can add it to the final list
								dbms_output.put_line( '        #' ||xx || ' is ' || a_neh_table(xx).neh_ne_id_new);
					...
					if n_old_ne_id = r_start_datum.old_ne_id_of then
						if final_datum.count <> 0 then 
							r_start_datum.new_ne_id_of := final_datum(final_datum.first).neh_ne_id_new;
						else 
					...
					if n_old_ne_id = r_end_datum.old_ne_id_of then
						if final_datum.count <> 0 then 
							r_end_datum.new_ne_id_of := final_datum(final_datum.last).neh_ne_id_new;
						else 
...
************************************************************* */
				...
				begin
					...
					if b_debug = true then 
						...
						dbms_output.put_line('---------------Starting Get Old List for: ' || n_old_ne_id || ', ' || 'is start: ' || s_temp);
					end if;
						...
						if spilt_neh_table.count <> 0 then 
							...
							for yy in spilt_neh_table.first..spilt_neh_table.last loop
								...
								if process_spilt_neh_table.count <> 0 then 								
									...
									if b_debug = true then 
										dbms_output.put_line('          No Additional  History for ' || spilt_neh_table(yy).neh_ne_id_new || ' Putting it into a_neh_table to return' );
									end if;
									a_neh_table.extend(1);  -- Add what we sent to split processing in if nothing was return
									a_neh_table(a_neh_table.count) := spilt_neh_table(yy);
									null;
...
--					
--					with rt as (
--									select /*+ materialize */ * from xaa_route_temp_sql
--								)
--					select count(*) into n_count from (
--						select distinct a.ne_id_of, a.slk, a.end_slk  from rt a, nm_elements , (select min(slk) min_slk from rt where ne_id_in = n_rt) b
--										where ne_id_of = ne_id 
--										AND A.ne_id_in = n_rt
--										and slk <= min_slk + n_offset_len	
--										and end_slk > min_slk + n_offset_len
--										order by slk
--										);
--					n_count := 0;
--					
--					if b_debug = true then
--							s_log_base_info := 'OFFSET: Begin check had a count of: ' || n_count;
--							s_log_text := 'The offset was: ' || n_offset_len;
--							x_log_table.log_item(s_module, s_log_area, s_log_base_info , s_log_text );
--						end if;
--					
--					if n_count = 1 then
...
--								end if;
--								
--								--if r_row.slk >= n_spilt_slk then --See if were are starting form the right place wiht al the spilts that occurred
...
--				THEN
--					null;
--				end;  --ends this block for error trapping
--							
--							--lets do the end
--				-- If it is a point asset lets keep the beginning only and delete the rest
--				
--				if r_input.offset_from = r_input.offset_to then
--					-- change the date to be point only
--					update xaa_route_temp_sql 
--							set end_slk = slk + n_offset_len
--							--set end_slk = 100+slk + n_offset_len
--							where ne_id_of = r_start_datum.new_ne_id_of;
--					
--					-- remove the other entries
--					delete from xaa_route_temp_sql where ne_id_of <> r_start_datum.new_ne_id_of;
--					
--				else
--				
--					begin 
...
--								select /*+ materialize */ * from xaa_route_temp_sql
--							)
--							select count(*) into n_count from (select distinct a.ne_id_of, a.slk, a.end_slk  from rt a, nm_elements , (select max(end_slk) max_end_slk from rt where ne_id_in = n_rt) b
--										where ne_id_of = ne_id 
--										AND A.ne_id_in = n_rt
--										and end_slk >= max_end_slk - n_offset_len
--										and max_end_slk - n_offset_len = abs(max_end_slk - n_offset_len)
--										and slk < max_end_slk - n_offset_len
--										order by slk);
--						
--						if b_debug = true then
--							s_log_base_info := 'OFFSET END DATUM N_COUNT';
--							s_log_text := n_count;
--							x_log_table.log_item(s_module, s_log_area, s_log_base_info , s_log_text );
--						end if;
--						
...
--														select /*+ materialize */ * from xaa_route_temp_sql
--														)
--													select distinct a.ne_id_of, a.slk, a.end_slk  from rt a, nm_elements , (select max(end_slk) max_end_slk from rt where ne_id_in = n_rt) b
--															where 1=1
--															--and ne_id_of = ne_id 
--															AND A.ne_id_in = n_rt
--															and end_slk >= max_end_slk - n_offset_len						
			...
			for r_input in c_route_list loop
				begin
					...
					*/
					...
					if b_debug = true then
						s_log_base_info := 'start datum slk end datum slk ..  Offsets';
						s_log_text := r_start_datum.old_ne_id_of || ', ' ||  r_end_datum.old_ne_id_of || '  *** ' || r_start_datum.datum_start_slk || ', ' ||  r_end_datum.datum_end_slk || '  *** ' || r_start_datum.datum_offset || ', ' || r_end_datum.datum_offset;
						x_log_table.log_item(s_module, s_log_area, s_log_base_info , s_log_text );
					...
					if r_input.offset_from = r_input.offset_to then
						n_code := 7;  -- Item is a point asset and will process a bit differently
					else					
