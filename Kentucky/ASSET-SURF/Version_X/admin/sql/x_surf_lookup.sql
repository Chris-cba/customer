...
CREATE OR REPLACE function x_surf_lookup_n(p_pm_type number, p_bd_type number) return number
   ...
   begin
        ...
        Elsif p_bd_type is not null then
            n_temp := p_bd_type;
            select surf_lookup  into n_temp from x_tab_surf_lookup where rownum =1 and pave_type = 'BD' and code =  (select ne_sub_type from nm_elements where ne_id = p_bd_type);
        Else 
            n_temp := -1;
        end if;
...
CREATE OR REPLACE function x_surf_lookup_bd(p_pm_type number, p_bd_type number) return number
   is
   n_temp number(8);
   s_desc varchar2(188);
   begin
        ...
        if  p_pm_type is not null then
            n_temp := null;
        Elsif p_pm_type is  null then
            --n_temp := p_bd_type;
            select ne_sub_type into  n_temp  from nm_elements where ne_id = p_bd_type;
        Else 
            n_temp := -1;
        end if;
