declare
	cursor c1 is select * from NM_INV_TYPES_ALL where NIT_INV_TYPE not in('HATH', 'HATT') and NIT_INV_TYPE not in (select iit_chr_attrib26 from nm_inv_items where iit_inv_type = 'HATH' and iit_end_date is null);
	...
	/*
		insert into nm_inv_items 
			...
			*/
            ...
            p_invrow.iit_inv_type := 'HATH' ;
            p_invrow.iit_start_date := trunc(sysdate);
            p_invrow.iit_date_created := trunc(sysdate);
			p_invrow.iit_descr := substr('HATH Information for ' || rec_att.nit_inv_type ||'-'||rec_att.nit_descr, 1,40);
