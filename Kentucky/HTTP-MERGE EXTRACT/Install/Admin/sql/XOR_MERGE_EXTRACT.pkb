...
BEGIN
  ...
  nm3web.module_startup(c_this_module);
      ...
      htp.formopen(curl => 'EXOR.' || g_package_name || '.report');
...
nm_debug.debug('in the second procedure');
  ...
  l_rec_nuf.name                   := 'Merge_extract_'||pi_selected_result || '_' ||pi_selected_query || '_' ||to_char(sysdate,'YYYY-MM-DD:HH24:MI:SS')||'.csv'  ;
  
   select distinct NQR_DESCRIPTION, to_char(nqr_date_created,'YYYY-MM-DD_HH24:MI:SS')   into l_result_name, l_last_ran from nm_mrg_query_results, nm_mrg_query
...
h_delim :=  case pi_delim
  ...
  l_title :=  'Merge Query: ' || pi_selected_query|| chr(13)||chr(10) ||'Result Set: ' ||l_result_name || chr(13)||chr(10) || 'Result Set Created On: ' || l_last_ran || chr(13)||chr(10);
   l_table := 'V_MRG_'|| pi_selected_query ||'_SVL';
   l_where := ' where NQR_MRG_JOB_ID = ' || pi_selected_result;
   l_order := ' order by RT_UNIQUE, BEGIN_MP ';
...
/*
       csql :=   'Select
                             ...
                             || l_delim || '||''"''' || '|| GET_NMS_NE_UNIQUE_FIRST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID)' || '||''"'''
                             || l_delim || '|| GET_NMS_BEGIN_MP_FIRST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID)'
                             || l_delim || '||''"''' || '|| GET_NMS_NE_UNIQUE_LAST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID) '   || '||''"'''
                             || l_delim || '|| GET_NMS_END_MP_LAST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID)'
                             || l_delim || '|| GET_MRG_SECTION_LENGTH (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID)'
                             || l_delim ;
      */
...
for n in 
 ...
 select column_name a
        ...
        l_qry := 'Select nm3net.get_ne_unique(NMS_OFFSET_NE_ID) RT_UNIQUE, NMS_BEGIN_OFFSET BEGIN_MP, NMS_END_OFFSET  END_MP, '''|| l_last_ran  ||'''  EXTR_DATE, ' ; 
        
        
        -- For RMS only
        /*
          l_qry := l_qry || 'GET_NMS_NE_UNIQUE_FIRST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID) S_UNIQ 
                        , GET_NMS_BEGIN_MP_FIRST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID) SLDI
                        ,  GET_NMS_NE_UNIQUE_LAST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID) E_UNIQ
                       ,  GET_NMS_END_MP_LAST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID) ELDI
                       , GET_MRG_SECTION_LENGTH (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID) SEG_LENGTH
                       , ' ;
           */
        ...
        l_qry := 'select * from (' || l_qry || ') ' || l_order;
...
select  l_title
    ...
    */
    ...
    v_row :=  'RT_UNIQUE' || h_delim || 'BEGIN_MP' || h_delim || 'END_MP' || h_delim || 'EXTR_DATE' || h_delim || l_header || '';
...
htp.p('<h2> ' || l_title || '</h2>');
    ...
    if l_tab.count > 50 then
        htp.p('There are ' ||( l_tab.count - 2) || ' rows in the result set so the full data set is not listed here. <P>  Only the
       first 50 rows are listed below and the full set is contained in the CSV file. <p> <p>');

    l_qry := 'select * from (' || l_qry ||' )  where rownum < 51 ';
