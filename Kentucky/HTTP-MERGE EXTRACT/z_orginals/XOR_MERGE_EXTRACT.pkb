CREATE OR REPLACE PACKAGE BODY RAMS.XOR_merge_extract AS
--
 -- R. Ellis - Sept. 2011
...
BEGIN
  ...
  nm3web.module_startup(c_this_module);
   ...
   htp.formopen(curl => 'RAMS.' || g_package_name || '.rep_params_2');
...
BEGIN
  ...
  nm3web.module_startup(c_this_module);
      ...
      htp.formopen(curl => 'RAMS.' || g_package_name || '.report');
  ...
  l_title :=  'Merge Query: ' || pi_selected_query|| ' Result Set: ' ||l_result_name ;
   ...
   l_where := ' where NQR_MRG_JOB_ID = ' || pi_selected_result;
       ...
       csql :=   'Select
                             ...
                             || l_delim || '||''"''' || '|| XRTA_MRG_EXTRACT.GET_NMS_NE_UNIQUE_FIRST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID)' || '||''"'''
                             || l_delim || '|| XRTA_MRG_EXTRACT.GET_NMS_BEGIN_MP_FIRST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID)'
                             || l_delim || '||''"''' || '|| XRTA_MRG_EXTRACT.GET_NMS_NE_UNIQUE_LAST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID) '   || '||''"'''
                             || l_delim || '|| XRTA_MRG_EXTRACT.GET_NMS_END_MP_LAST (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID)'
                             || l_delim || '|| XRTA_MRG_EXTRACT.GET_MRG_SECTION_LENGTH (NQR_MRG_JOB_ID,NMS_MRG_SECTION_ID)'
                             || l_delim ;
