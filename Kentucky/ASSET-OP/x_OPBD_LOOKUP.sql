CREATE OR REPLACE function x_OPBD_LOOKUP_CHAR(p_what varchar2, p_job_id number, p_section_ID  number) return varchar2
   ...
   Case upper(p_what)
	When 'NE_UNIQUE'
		then
			select NMS_OFFSET_NE_ID  into n_temp from V_MRG_TYPEOP where rownum = 1 and nqr_mrg_job_id = p_job_id and nms_mrg_section_id = p_section_ID;
			select ne_unique into s_desc from nm_elements where ne_id = n_temp;
     
     
	When 'DISTRICT'
		then
			select nqr_source_id  into n_temp from V_MRG_TYPEOP where rownum = 1 and nqr_mrg_job_id = p_job_id and nms_mrg_section_id = p_section_ID;
...
CREATE OR REPLACE function x_OPBD_LOOKUP_NUM (p_what varchar2, p_job_id number, p_section_ID  number) return number
   ...
   Case upper(p_what)
	When 'BMP' then 
	select NMS_BEGIN_OFFSET  into n_temp_dp from V_MRG_TYPEOP where rownum = 1 and nqr_mrg_job_id = p_job_id and nms_mrg_section_id = p_section_ID;
     
     return n_temp_dp;
	When 'EMP' then
	select NMS_END_OFFSET  into n_temp_dp from V_MRG_TYPEOP where rownum = 1 and nqr_mrg_job_id = p_job_id and nms_mrg_section_id = p_section_ID;
