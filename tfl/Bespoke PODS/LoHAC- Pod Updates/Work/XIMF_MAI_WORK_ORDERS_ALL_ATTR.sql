DROP VIEW HIGHWAYS.XIMF_MAI_WORK_ORDERS_ALL_ATTR;

/* Formatted on 10/7/2013 4:21:34 PM (QP5 v5.256.13226.35538) */
CREATE OR REPLACE FORCE VIEW HIGHWAYS.XIMF_MAI_WORK_ORDERS_ALL_ATTR
(
   WORKS_ORDER_NUMBER,
...
AS
   SELECT -------------------------------------------------------------------------
                                                      --   PVCS Identifiers :-
                                                                            --
 --       PVCS id          : $Header:   //new_vm_latest/archives/customer/tfl/Bespoke PODS/LoHAC- Pod Updates/Work/XIMF_MAI_WORK_ORDERS_ALL_ATTR.sql-arc   1.0   Jan 14 2016 22:34:28   Sarah.Williams  $
 --       Module Name      : $Workfile:   XIMF_MAI_WORK_ORDERS_ALL_ATTR.sql  $
                  --       Date into PVCS   : $Date:   Jan 14 2016 22:34:28  $
               --       Date fetched Out : $Modtime:   Oct 08 2013 21:14:28  $
     ...
     -------------------------------------------------------------------------
         ...
         NVL (
            (SELECT hco_meaning
               FROM hig_codes
              WHERE hco_domain = 'WOR_PRIORITY' AND hco_code = wor.wor_priority),
            (SELECT wpr_name
         ...
         (SELECT hco_meaning
            FROM hig_codes
           WHERE hco_domain = 'ROAD_SYS_FLAG' AND hco_code = wor.wor_sys_flag)
            sys_flag_description,
         wor_scheme_type scheme_type,
         (SELECT hco_meaning
            FROM hig_codes
           WHERE hco_domain = 'SCHEME_TYPES' AND hco_code = wor.wor_scheme_type)
            scheme_type_description,
   ...
   WHERE     con.con_id = wor.wor_con_id
         AND (   (x_get_im_user_id = 1)
              OR (   NOT EXISTS
                        (SELECT 1
                           FROM mai_wo_users
                          WHERE mwu_user_id = x_get_im_user_id --(select hus_user_id from hig_users where hus_username = v('APP_USER'))
                                                              )
                  OR wor.wor_works_order_no IN (SELECT wor_works_order_no
                                                  FROM work_orders
                                                 WHERE wor_forwarded_to =
                                                          x_get_im_user_id
...
CREATE OR REPLACE PUBLIC SYNONYM XIMF_MAI_WORK_ORDERS_ALL_ATTR FOR HIGHWAYS.XIMF_MAI_WORK_ORDERS_ALL_ATTR;
