  ...
  SELECT    '<a href="javascript:openForms(''WORK_ORDERS'','''
          ...
          (CASE
           ...
           END)
             due_date,
           (CASE
              WHEN WOR_DATE_ATTRIB121 is not null AND substr(con_code,instr(con_code,'_')+1) in ('HLSC', 'HLSR', 'SLSC', 'SLSR')
              THEN
					WOR_DATE_ATTRIB121
				WHEN WOR_DATE_ATTRIB121 is null AND substr(con_code,instr(con_code,'_')+1) in ('HLSC', 'HLSR', 'SLSC', 'SLSR')
              THEN	
                 DEF_INSPECTION_DATE + (WOR_DATE_CONFIRMED - WOL_DATE_REPAIRED)  
              WHEN WOR_DATE_ATTRIB121 is not null AND substr(con_code,instr(con_code,'_')+1) in ('HR', 'HTO', 'SMCI', 'SR', 'STO')
				THEN
					WOR_DATE_ATTRIB121
              WHEN WOR_DATE_ATTRIB121 is null AND substr(con_code,instr(con_code,'_')+1) in ('HR', 'HTO', 'SMCI', 'SR', 'STO')
				THEN
                 WOR_DATE_CONFIRMED --+ (wor.wor_est_complete)-- not complete
              ELSE
                 wor.wor_est_complete + 1 - (1 / (24 * 60 * 60))
           END) target_date,
          (SELECT wor_status
    ...
    WHERE     NVL (WOR_CHAR_ATTRIB100, 'Empty') NOT IN ('REJ', 'HLD')
          ...
          AND 
					...
					ELSE
					 wor.wor_est_complete + 1 - (1 / (24 * 60 * 60))
					END)		  >= (select min(st_range) from X_LOHAC_DateRANGE_WODC)
