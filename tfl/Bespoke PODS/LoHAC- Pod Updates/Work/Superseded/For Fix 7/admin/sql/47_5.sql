...
if xtfl_wo_trg_pkg.is_user_non_tfl() and :NEW.wor_char_attrib110 in (XTFL_WO_TRG_PKG.GET_INV_DOMAIN_VALUE('INVOICE_STATUS', 'Approved'),XTFL_WO_TRG_PKG.GET_INV_DOMAIN_VALUE('INVOICE_STATUS', 'Rejected')) then
--change for revised old remains old
:NEW.wor_char_attrib110 := :OLD.wor_char_attrib110;
-- date added for Invoice Status Effective Date
:NEW.WOR_DATE_ATTRIB131 := trunc(sysdate);
--:NEW.wor_char_attrib110 := null;
elsif xtfl_wo_trg_pkg.is_non_commercial_team() and :NEW.wor_char_attrib110 = XTFL_WO_TRG_PKG.GET_INV_DOMAIN_VALUE('INVOICE_STATUS', 'Rejected') 
then
